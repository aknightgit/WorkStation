...
CREATE PROC  [dw].[SP_Fct_YH_Sales_Inventory_20191018]
AS 
BEGIN
...
FROM(
  ...
  WHERE calday>='20190701'
	  ...
	  LEFT JOIN DM.Dim_Product Prod ON sal.[YH_UPC] = Prod.Bar_Code AND CASE WHEN sal.SKU_NM LIKE '%小猪%' THEN 'PEPPA' WHEN sal.SKU_NM LIKE '%瑞奇%' THEN 'RIKI' ELSE 'PEPPA' END = CASE WHEN prod.Brand_IP IN ('PEPPA','RIKI') THEN prod.Brand_IP ELSE 'PEPPA' END
...
INSERT INTO dw.[Fct_YH_Sales_Inventory](
 ...
 SELECT NULL AS  [YH_Type]    
	   ...
	   LEFT JOIN DM.Dim_Product Prod ON ISNULL(ISNULL(Sales.[YH_UPC],Inv.[YH_UPC]),InvLD.[YH_UPC]) = Prod.Bar_Code AND CASE WHEN ISNULL(ISNULL(Sales.SKU_NM,Inv.SKU_NM),InvLD.SKU_NM) LIKE '%小猪%' THEN 'PEPPA' WHEN ISNULL(ISNULL(Sales.SKU_NM,Inv.SKU_NM),InvLD.SKU_NM) LIKE '%瑞奇%' THEN 'RIKI' ELSE 'PEPPA' END = CASE WHEN prod.Brand_IP IN ('PEPPA','RIKI') THEN prod.Brand_IP ELSE 'PEPPA' END

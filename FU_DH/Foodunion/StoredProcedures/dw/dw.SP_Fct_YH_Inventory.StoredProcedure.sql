USE [Foodunion]
GO
DROP PROCEDURE [dw].[SP_Fct_YH_Inventory]
GO
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE  [dw].[SP_Fct_YH_Inventory]
AS
BEGIN
...
BEGIN TRY 
  ...
  SELECT NULL AS [YH_Type],
		 ...
		 LEFT JOIN dm.Dim_Product P ON T.[YH_UPC] = P.Bar_Code AND CASE WHEN t.SKU_NM LIKE '%Ð¡Öí%' THEN 'PEPPA' WHEN t.SKU_NM LIKE '%ÈðÆæ%' THEN 'RIKI' ELSE 'PEPPA' END = CASE WHEN p.Brand_IP IN ('PEPPA','RIKI') THEN p.Brand_IP ELSE 'PEPPA' END
  ...
  SELECT NULL AS [YH_Type],
		 ...
		 LEFT JOIN dm.Dim_Product P ON T.bar_code = P.Bar_Code AND CASE WHEN t.goods_name LIKE '%Ð¡Öí%' THEN 'PEPPA' WHEN t.goods_name LIKE '%ÈðÆæ%' THEN 'RIKI' ELSE 'PEPPA' END = CASE WHEN p.Brand_IP IN ('PEPPA','RIKI') THEN p.Brand_IP ELSE 'PEPPA' END

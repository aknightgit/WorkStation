USE [Foodunion]
GO
DROP PROCEDURE [dbo].[SP_DIM_Product_Table_SP_Updater]
GO
SET ANSI_NULLS ON
GO
...
CREATE   PROCEDURE [dbo].[SP_DIM_Product_Table_SP_Updater] 
...
SET @QUERY+= 'File_NM NVARCHAR(200),Update_DTM datetime,Load_DTM datetime)'
...
SET @QUERY+='[File_NM],' +CHAR(13)+CHAR(10)
...
WHERE 
 COL.[name]<>'File_NM' AND COL.[name]<>'Load_DTM' and
 COL.[object_id] in (SELECT  [object_id] FROM SYS.OBJECTS WHERE [type]='U' and [name] =@TableName)
SET @QUERY+='T.[File_NM] =S.[File_NM], ' +CHAR(13)+CHAR(10)
...
SET @QUERY+='S.[File_NM],' +CHAR(13)+CHAR(10)

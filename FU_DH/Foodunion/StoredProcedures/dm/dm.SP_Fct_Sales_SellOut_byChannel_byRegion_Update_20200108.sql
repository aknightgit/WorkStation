...
GO
CREATE PROCEDURE  [dm].[SP_Fct_Sales_SellOut_byChannel_byRegion_Update_20200108] --400
	@Ret_Days INT = 90
AS
BEGIN
 ...
 ------------------------------------------------根据明细拆分SKU-------------------------------------------Justin 2020-01-08
   ...
   GROUP BY CONVERT(VARCHAR(10),[Order_Creation_Time],112),DD.SKU_ID
	...
	-------------------------CP拿不到sellout 数据， 用sellin 代替   
	                       --增加 深圳微之家是EKA的经销商渠道，用SellIn做SellOut    Justin 2020-01-08
	UNION ALL
	...
	JOIN dm.Dim_Channel dc WITH(NOLOCK) ON si.Channel_ID = dc.Channel_ID AND (dc.Channel_Type = 'CP' OR dc.Channel_Name_Display = '深圳微之家' )

USE [Foodunion]
GO
DROP PROCEDURE [dm].[SP_Fct_YH_Store_Flag_Daily]
GO
SET ANSI_NULLS ON
GO
...
CREATE PROC  [dm].[SP_Fct_YH_Store_Flag_Daily]
@Total INT = 0
AS
BEGIN
...
LEFT JOIN #YH_Sales_Inventory AS sal ON dt.Date_ID = sal.Calendar_DT AND st.Store_ID = sal.Store_ID
...
(		SELECT dt.Date_ID
		...
		FROM [FU_EDW].[Dim_Calendar] dt
		CROSS JOIN [dm].[Dim_Store] st
		LEFT JOIN #YH_Sales_Inventory AS si ON dt.Date_ID >= si.Calendar_DT AND DATEDIFF(DAY,CAST(si.Calendar_DT AS VARCHAR),dt.Date_NM)<3 AND st.Store_ID = si.Store_ID
		WHERE dt.Date_ID>@MAX_CAL AND dt.Date_NM<GETDATE() AND  Status = N'营运' AND st.Channel_Account = 'YH' --AND stf.FIRST_SALES_DATE IS NOT NULL -- AND dt.Date_ID NOT IN (SELECT DISTINCT sfd.Date_ID FROM FU_DM.T_DM_FCT_YH_Store_Flag_Daily sfd)-- AND Target_Store_FL=1 
		GROUP BY dt.Date_ID
				,dt.Date_NM
...
(
		...
		FROM [FU_EDW].[Dim_Calendar] dt
		CROSS JOIN [dm].[Dim_Store] st
		LEFT JOIN #YH_Sales_Inventory AS si ON dt.Date_ID >= si.Calendar_DT AND DATEDIFF(DAY,CAST(si.Calendar_DT AS VARCHAR),dt.Date_NM)<14 AND st.Store_ID = si.Store_ID
		WHERE dt.Date_ID>@MAX_CAL AND dt.Date_NM<GETDATE() AND  Status = N'营运' AND st.Channel_Account = 'YH' --AND stf.FIRST_SALES_DATE IS NOT NULL -- AND dt.Date_ID NOT IN (SELECT DISTINCT sfd.Date_ID FROM FU_DM.T_DM_FCT_YH_Store_Flag_Daily sfd)-- AND Target_Store_FL=1 
		GROUP BY dt.Date_ID
				,dt.Date_NM
				,st.Account_Store_Type
...
(
		...
		FROM [FU_EDW].[Dim_Calendar] dt
		CROSS JOIN [dm].[Dim_Store] st
		LEFT JOIN #YH_Sales_Inventory AS sal ON dt.Date_ID >= sal.Calendar_DT AND DATEDIFF(DAY,CAST(sal.Calendar_DT AS VARCHAR),dt.Date_NM)<14 AND st.Store_ID = sal.Store_ID
		WHERE dt.Date_ID>@MAX_CAL AND dt.Date_NM<GETDATE() AND  Status = N'营运' AND st.Channel_Account = 'YH' --AND stf.FIRST_SALES_DATE IS NOT NULL -- AND dt.Date_ID NOT IN (SELECT DISTINCT sfd.Date_ID FROM FU_DM.T_DM_FCT_YH_Store_Flag_Daily sfd)-- AND Target_Store_FL=1 
		GROUP BY dt.Date_ID
				,dt.Date_NM
				,st.Account_Store_Type
...
(
		...
		FROM [FU_EDW].[Dim_Calendar] dt
		CROSS JOIN [dm].[Dim_Store] st
		LEFT JOIN #YH_Sales_Inventory AS sal ON dt.Date_ID >= sal.Calendar_DT AND DATEDIFF(DAY,CAST(sal.Calendar_DT AS VARCHAR),dt.Date_NM)<7 AND st.Store_ID = sal.Store_ID
		WHERE dt.Date_ID>@MAX_CAL AND dt.Date_NM<GETDATE() AND  Status = N'营运' AND st.Channel_Account = 'YH' --AND stf.FIRST_SALES_DATE IS NOT NULL -- AND dt.Date_ID NOT IN (SELECT DISTINCT sfd.Date_ID FROM FU_DM.T_DM_FCT_YH_Store_Flag_Daily sfd)-- AND Target_Store_FL=1 
		GROUP BY dt.Date_ID
				,dt.Date_NM
				,st.Account_Store_Type
...
(
		...
		FROM [FU_EDW].[Dim_Calendar] dt
		CROSS JOIN [dm].[Dim_Store] st
		LEFT JOIN #YH_Sales_Inventory AS sal ON DATEDIFF(DAY,CAST(sal.Calendar_DT AS VARCHAR),dt.Date_NM)>=7 AND DATEDIFF(DAY,CAST(sal.Calendar_DT AS VARCHAR),dt.Date_NM)<14 AND st.Store_ID = sal.Store_ID
		WHERE dt.Date_ID>@MAX_CAL AND dt.Date_NM<GETDATE() AND  Status = N'营运' AND st.Channel_Account = 'YH' --AND stf.FIRST_SALES_DATE IS NOT NULL -- AND dt.Date_ID NOT IN (SELECT DISTINCT sfd.Date_ID FROM FU_DM.T_DM_FCT_YH_Store_Flag_Daily sfd)-- AND Target_Store_FL=1 
		GROUP BY dt.Date_ID
				,dt.Date_NM
				,st.Account_Store_Type
...
(
		...
		WHERE dt.Date_ID>@MAX_CAL AND dt.Day_Filter_Flag =1
		GROUP BY dt.Date_ID
			  ,st.Store_ID 

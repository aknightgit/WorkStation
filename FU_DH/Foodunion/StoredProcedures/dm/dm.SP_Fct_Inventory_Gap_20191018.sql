...
WHERE wh.Warehouse_Name LIKE '%恒知%' AND ISNULL(CAST(minv.Inventory_QTY AS DECIMAL),0) >0.000 
UNION ALL
...
LEFT JOIN [Foodunion].[dm].[Dim_Warehouse] wh ON REPLACE(REPLACE(einv.Stock_Name,'常温',''),'冷藏','')	= wh.Warehouse_Name
WHERE wh.Warehouse_Name LIKE '%恒知%' AND ISNULL(CAST(einv.Sale_QTY AS DECIMAL),0) >0.000 
) base
GROUP BY
       [Inventory_DT]
      ,[Warehouse_ID]

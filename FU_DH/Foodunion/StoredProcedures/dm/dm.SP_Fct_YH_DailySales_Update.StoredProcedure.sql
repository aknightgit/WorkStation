USE [Foodunion]
GO
DROP PROCEDURE [dm].[SP_Fct_YH_DailySales_Update]
GO
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dm].[SP_Fct_YH_DailySales_Update]
AS
BEGIN		
	...
	BEGIN TRY
		...
		TRUNCATE TABLE [dm].[Fct_YH_DailySales]
           
		INSERT INTO [dm].[Fct_YH_DailySales]
           ([Datekey]
		   ,[Store_ID]
		   ,[SKU_ID]
		   ,[InStock_QTY]
		...
		LEFT JOIN dm.Dim_Product prod ON ds.Bar_Code=prod.Bar_Code AND CASE WHEN ds.SKU_Name LIKE '%Ð¡Öí%' THEN 'PEPPA' WHEN ds.SKU_Name LIKE '%ÈðÆæ%' THEN 'RIKI' ELSE 'PEPPA' END = CASE WHEN prod.Brand_IP IN ('PEPPA','RIKI') THEN prod.Brand_IP ELSE 'PEPPA' END

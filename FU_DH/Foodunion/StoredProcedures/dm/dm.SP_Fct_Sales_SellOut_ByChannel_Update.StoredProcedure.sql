USE [Foodunion]
GO
DROP PROCEDURE [dm].[SP_Fct_Sales_SellOut_ByChannel_Update]
GO
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	JOIN [dm].[Fct_O2O_Order_Detail_info] di WITH(NOLOCK) ON bi.Order_ID=di.Order_ID
	JOIN [dm].[Dim_Channel] dc WITH(NOLOCK) ON dc.Channel_Name='Youzan'
	JOIN [dm].[Dim_Product] p WITH(NOLOCK) ON di.SKU_ID=p.SKU_ID

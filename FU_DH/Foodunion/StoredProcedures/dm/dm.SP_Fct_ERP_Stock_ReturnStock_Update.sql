...
BEGIN		
  ...
  --          ,p.[Bill_Type]		 =c.[Bill_Type]
		...
		SELECT 
			...
			,ISNULL(CASE WHEN c.Base_Unit = prod.Sale_Unit_CN THEN c.Base_Unit_QTY ELSE c.Base_Unit_QTY*cr.Convert_Rate END,c.[Price_Unit_QTY] ) * prod.Sale_Unit_Weight_KG
			,c.[Price]
			,c.[TaxPrice]
			,c.[Price_Unit]
			,c.[Price_Unit_QTY]

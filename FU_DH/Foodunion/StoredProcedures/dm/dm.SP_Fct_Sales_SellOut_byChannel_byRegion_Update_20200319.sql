...
CREATE PROCEDURE  [dm].[SP_Fct_Sales_SellOut_byChannel_byRegion_Update_20200319] --400
   ...
   GROUP BY CONVERT(VARCHAR(10),[Order_Creation_Time],112),DD.SKU_ID
	...
	FROM(
		SELECT fo.Channel_ID,CONVERT(char(8),fo.Order_PayTime,112) Order_DateKey  --使用付款成功时间
			,foi.SKU_ID,sum(foi.Payment_Amount_Split) AS Payment 
...
*/
	...
	SELECT 
		  ...
		  ,''
		  ,sb.SKU_ID
		  ,SUM(CAST(sb.PCS AS INT) * CAST(SKU_Shipment_QTY AS INT))
		  ,SUM(CAST(sb.Actual_Payment_Price AS FLOAT))
		  ,SUM(CAST(sb.Actual_Payment_Price AS FLOAT))/1000

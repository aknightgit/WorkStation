...
CREATE PROC [dm].[SP_Fct_YouZan_StoreOrder_Update]
AS
BEGIN
...
*/
	...
	INSERT INTO [dm].[Fct_O2O_Order_Base_info]
           ...
           ,[Update_By])
    SELECT 
		CAST(CONVERT(VARCHAR(8),i.[created],112) AS INT) AS Datekey,
		i.tid as Order_ID,
		i.tid as Order_No,
		...
		NULL as Union_id,	
		CASE WHEN cy.item_type = 24 THEN 1 ELSE 0 END as is_cycle,
		i.status as Order_Status,		
		i.status_str as Order_Status_Str,
		CASE  i.pay_type WHEN 0 THEN '待付款' ELSE '已支付' END as Pay_Status,
		I.[pay_type_str] Pay_Type_Str,
		CASE i.pay_type 
		...
		i.[total_fee] as Order_Amount,
		i.[post_fee] as Shipping_Amount,
		i.[payment] as Pay_Amount,
		0 as Refund_Amount,
		i.[created] as Order_Create_Time,
		i.[success_time] as [Order_Close_Time],
	...
	FROM [ODS].[ods].[Youzan_Orders] i  
	LEFT JOIN (SELECT DISTINCT [tid],[item_type] FROM [ODS].[stg].[Youzan_OrderDetails]) cy ON  cy.tid=i.tid
	...
	LEFT JOIN dm.Dim_O2O_Fans fu WITH(NOLOCK) ON i.[outer_user_id] = fu.[open_id]
	LEFT JOIN #Fans_Order_cnt foc ON isnull(i.outer_user_id,'') = foc.outer_user_id
	--where i.fans_nickname='特困??生'
	WHERE CONVERT(VARCHAR(8),i.[created],112)>='20200801' ;

...
BEGIN		
	...
	BEGIN TRY
		...
		SELECT 
		    ...
		    ,c.Warehouse_ID
		    ,c.Warehouse_Name
			,CASE WHEN c.Unit = ISNULL(c.[SaleUnit],prod.Sale_Unit) THEN c.[QTY] ELSE c.[BaseUnitQTY] * isnull(uc.ConvertRate,1) END * prod.Sale_Unit_Weight_KG
			,GETDATE()
			,@ProcName
			,GETDATE()
			,@ProcName
		FROM ODS.[ods].[ERP_Sale_OrderEntry] c
		JOIN ODS.[ods].[ERP_Sale_Order] so ON c.[SaleOrderID] = so.[SaleOrderID]
		...
		LEFT JOIN [ODS].[ods].[ERP_Unit_ConvertRate] uc ON c.[BaseUnit] = uc.FromUnit AND c.[SaleUnit] = uc.ToUnit
		WHERE p.[Order_Entry_ID] IS NULL AND so.[BillNo] <>'';

...
AS BEGIN
	...
	WHERE (org_name in ('o2o','营养顾问') or employee_id in 
			(SELECT DISTINCT Fenxiao_Employee_id FROM ODS.ods.SCRM_order_base_info))  --未在mapping表，但是有分销单,
		AND mobile NOT IN (SELECT isnull(Mobile,'') FROM ods.ods.SCRM_O2O_QRCodeMapping WHERE QRCode NOT IN ('办公室','武康路'))  -- O2O Employee that not in QR Mapping list

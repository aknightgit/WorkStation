...
GO
CREATE PROC [dm].[SP_Fct_MiniSOP_Forecast_Update]
AS 
BEGIN
	...
	JOIN dm.Dim_Store s ON KA.Store_ID=s.Store_ID
	JOIN (
		SELECT Year,Week_of_Year,MAX(Datekey) Datekey from dm.Dim_Calendar
		GROUP BY Year,Week_of_Year
	)wkd ON wkd.Datekey = ka.Datekey
	--WHERE wkd.Year=2019 
	--AND wkd.Week_of_Year=37
	GROUP BY CASE WHEN s.Store_Province='内蒙古' THEN 'IM/内蒙'
			WHEN s.Store_Province='北京市' THEN 'BJ/北京'
			WHEN s.Store_Province='天津市' THEN 'TJ/天津'
			WHEN s.Store_Province='山东省' THEN 'SD/山东'
	...
	--JOIN dm.Dim_Store s ON KA.Store_ID=s.Store_ID
	JOIN (SELECT Year,Week_of_Year,Datepart(year,Dateadd(DAY,1,MAX([Date]))) AS nextweekyear,Datepart(week,Dateadd(DAY,1,MAX([Date]))) AS nextweek
		,Datepart(year,Dateadd(DAY,21,MAX([Date]))) AS next3weekyear,Datepart(week,Dateadd(DAY,21,MAX([Date]))) AS next3week
		FROM dm.Dim_Calendar 
		GROUP BY Year,Week_of_Year)d ON f.Year=d.Year AND f.Week_no=d.Week_of_Year
	GROUP BY  f.Period
		,f.Sales_Territory 
		,d.Year
		,d.Week_of_Year
	...
	JOIN #saleQTY tmp
	ON f.Year=tmp.Year 
		AND f.Week_No=tmp.Week_of_Year 
	...
	--select top 100 * from dm.Dim_Calendar where Week_of_Year=37

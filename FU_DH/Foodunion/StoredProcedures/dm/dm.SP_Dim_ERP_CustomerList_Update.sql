...
BEGIN		
	...
	BEGIN TRY
		...
		ON p.[Customer_ID] = c.[Customer_ID] AND p.Sequence_ID=c.Sequence_ID
		WHERE p.[Customer_ID] IS NULL;
		...
		UPDATE p
           ...
           ,p.[Country]=c.[Country]
           ,p.[Mobile]=c.[Mobile]
           ,p.[Default_Stock_ID]=c.[Default_Stock_ID]
           ,p.[Default_Stock_Name]=c.[Default_Stock_Name]
			,p.Update_Time=GETDATE()
			,p.Update_By= @ProcName
		FROM ODS.[ods].[ERP_Customer_Shipto] c
		JOIN [dm].[Dim_ERP_Customer_Shipto] p
		ON p.[Customer_ID] = c.[Customer_ID] AND p.Sequence_ID=c.Sequence_ID
		WHERE p.[Customer_Name] <> c.[Customer_Name]
           or p.[Ship_To] <> c.[Ship_To]
           or p.[Ship_To_Desc] <> c.[Ship_To_Desc]
           or p.[Address] <> c.[Address];

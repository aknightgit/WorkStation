...
CREATE PROCEDURE [dm].[SP_Fct_Order_Update_20200221]
AS
BEGIN
	...
	SELECT  CONVERT(VARCHAR(6),oi.add_time,112) AS [Order_MonthKey]
		...
		,CASE WHEN ISNUMERIC(oi.[user_name])=1 THEN oi.[user_name] ELSE NULL END AS [Buyer_Nick]
	...
	SET 
		...
		,[Buyer_Mobile] = CASE WHEN ISNUMERIC(oi.[user_name])=1 THEN oi.[user_name] ELSE NULL END 

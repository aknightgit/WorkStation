...
CREATE PROCEDURE [dm].[SP_Dim_Calendar_Update]
...
BEGIN
	...
	SELECT 
           ...
           ,(DATEPART(day,@Date)+DATEPART(weekday,DATEADD(DAY,-1,DATEADD(DAY, -1 * DATEPART(DAY, @Date) + 1, @Date)))-1+6)/7 AS [Week_of_Month]
	...
	UPDATE dm.Dim_Calendar set Is_Past=1 WHERE [DATE]<GETDATE()-1;

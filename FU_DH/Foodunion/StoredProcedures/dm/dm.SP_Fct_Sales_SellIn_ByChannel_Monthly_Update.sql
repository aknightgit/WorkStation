...
--SELECT COUNT(1) FROM [dm].[Fct_Sales_SellIn_ByChannel_Monthly]

--Modified  2020-11-09
--Change:  Set Zbox/DTC channel 出库单Net Sales，规则和其他渠道一样

CREATE PROCEDURE [dm].[SP_Fct_Sales_SellIn_ByChannel_Monthly_Update] 
AS
BEGIN
	...
	BEGIN TRY
	    ...
	    TRUNCATE TABLE [dm].[Fct_Sales_SellIn_ByChannel_Monthly]
		INSERT INTO [dm].[Fct_Sales_SellIn_ByChannel_Monthly]
			  ([Monthkey]
			  ,[Datekey]
			  ,[Customer_ID]
			  ,[Customer_Name]
			  ...
			  ,[Update_time]
			  ,[Update_By])        
		SELECT LEFT([Datekey],6), [Datekey],Customer_ID,Customer_Name,Channel_ID,Channel_Type,Channel_Name_Short,Channel_FIN,SubChannel_FIN, 
			   SUM([出库单未税]) AS [出库单未税],
			   SUM([FOC未税]) AS [FOC未税],
			   SUM([调拨单金额]) AS [调拨单金额],
			   SUM([退货单未税]) AS [退货单未税],
			   ...
			   SUM([退货单吨数]) AS [退货单吨数],
			   GETDATE() [Create_time],
			   '[dm].[Fct_Sales_SellIn_ByChannel_Monthly]' [Create_By],
			   GETDATE() [Update_time],
			   '[dm].[Fct_Sales_SellIn_ByChannel_Monthly]' [Update_By]
			   
		FROM ( 
				--2020/11/09  统一所有渠道的出库计算
		SELECT sos.[Datekey],
		...
		SELECT   
			...
			,0 AS [退货单未税]
			--,SUM(stie.Sale_QTY * pl.SKU_Price) AS [Net_Sales]
			,0 AS [Net_Sales]
			,0 AS [出库单含税]
			,0 AS [FOC含税]
			,0 AS [退货单含税]
			--,SUM(stie.Sale_QTY * pl.SKU_Price_withTax) AS [Net_Sales_wTax]
			,0 AS [Net_Sales_wTax]
			,0 AS [出库单非FOC吨数]
			,0 AS [FOC吨数]
		...
		GROUP BY LEFT([Datekey],6),[Datekey],Customer_ID,Customer_Name,Channel_ID,Channel_Type,Channel_Name_Short,Channel_FIN,SubChannel_FIN
	   ;

	   --Zbox 202006月份调单，不显示
	   DELETE FROM [dm].[Fct_Sales_SellIn_ByChannel_Monthly]
	...
	--select * into [dm].[Fct_Sales_SellIn_ByChannel_Monthly_20201105]
	--from [dm].[Fct_Sales_SellIn_ByChannel_Monthly]

	--exec dbo.USP_Change_TableColumn '[dm].[Fct_Sales_SellIn_ByChannel_Monthly]','add','Net_Sales_wTax decimal(18,2)','退货单含税',0
	
	--exec dbo.USP_Change_TableColumn '[dm].[Fct_Sales_SellIn_ByChannel_BySKU]','add','Net_Sales_wTax decimal(18,2)','退货单含税',0
GO

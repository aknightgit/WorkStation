USE [Foodunion]
GO
DROP PROCEDURE [dm].[SP_Fct_Kidswant_DailySales_Update]
GO
SET ANSI_NULLS ON
GO
...
SELECT CONVERT(VARCHAR(8),CAST(Date AS DATE),112) AS DATAKEY
  ...
  LEFT JOIN dm.Dim_Product prod ON sal.Bar_Code = prod.Bar_Code AND CASE WHEN sal.SKU_Name LIKE '%Ð¡Öí%' THEN 'PEPPA' WHEN sal.SKU_Name LIKE '%ÈðÆæ%' THEN 'RIKI' ELSE 'PEPPA' END = CASE WHEN prod.Brand_IP IN ('PEPPA','RIKI') THEN prod.Brand_IP ELSE 'PEPPA' END

...
GO
CREATE PROCEDURE  [dm].[SP_Fct_Sales_SellOut_byChannel_byRegion_Update_20200409] --400
...
BEGIN
	...
	SELECT ka.[DateKey]  AS [DateKey]
		...
		,'' AS Channel                           --增加字段，判断PDD 牧场订单   Justin 2020-04-09
 ...
 --	WHERE bi.Order_Status <> 'TRADE_CLOSED'
   ...
   GROUP BY CONVERT(VARCHAR(10),[Order_Creation_Time],112),DD.SKU_ID
	...
	SELECT ISNULL(a.Order_DateKey,b.refund_date) AS Datekey
		...
		,a.Channel                         --增加字段，判断PDD 牧场订单   Justin 2020-04-09
		,'' AS [Sale_Area]		
		,'' AS [Sale_Territory]
	...
	FROM(
		SELECT fo.Channel_ID
		     ,CASE WHEN fo.Channel_ID IN (71) AND foi.Payment_Amount_Split=0 AND foi.SKU_ID IN ('1181001','1120003') THEN 999 ELSE fo.Channel_ID END  AS Channel    --增加字段，判断PDD 牧场订单   Justin 2020-04-09
		     ,CONVERT(char(8),fo.Order_PayTime,112) Order_DateKey  --使用付款成功时间
			,foi.SKU_ID
			,SUM(--拼多多 多多牧场零元购订单折算
		...
		GROUP BY fo.Channel_ID,CONVERT(char(8),fo.Order_PayTime,112),foi.SKU_ID,
		      CASE WHEN fo.Channel_ID IN (71) AND foi.Payment_Amount_Split=0 AND foi.SKU_ID IN ('1181001','1120003') THEN 999 ELSE fo.Channel_ID END)a
	FULL JOIN (
	...
	JOIN dm.Dim_Channel c ON ISNULL(a.Channel_ID,b.Channel_ID)=c.Channel_ID AND c.Channel_Category IN ('EC-Tmall','EC-PDD')
...
*/
	...
	JOIN dm.Dim_Channel dc WITH(NOLOCK) ON si.Channel_ID = dc.Channel_ID 
		AND (dc.Channel_Type = 'CP' OR dc.Channel_ID IN (96,127) ) -- dc.Channel_Name_Display IN ('深圳微之家','嗨团') 

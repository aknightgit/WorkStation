...
CREATE PROCEDURE [rpt].[SP_RPT_Sales_YH_MonthlyPerformance_Overview_20200413]
AS BEGIN 
...
INTO #AreaMapping
FROM (
	SELECT DISTINCT bk.Monthkey,stm.SalesTerritory,bk.Area,ds.Store_Province
	--,bk.TargetAmt
...
GROUP BY  DATEKEY,Store_Province
 ...
 SELECT Datekey,Sales_Area_CN,SUM(Amount) AS Amount,SUM(TARGET) AS Target  INTO #SellIn
FROM (
SELECT  LEFT([Purchase_Datekey],6) AS Datekey    
	  ,S.Sales_Area_CN	 	  
	  --,SUM(([Purchase_QTY]/PP.Qty_SaleInTray)*P.SKU_Price) AS Amount 
	  ,SUM(([Purchase_QTY])* P.SKU_Base_Price)  AS Amount 
	  --,SUM(([Purchase_QTY])*ISNULL(P.Base_Price_withTax,PP.Sale_Unit_RSP)) AS Amount 
	  ,0 AS TARGET
  FROM [Foodunion].[dm].[Fct_YH_Store_DailyOrders] O
  LEFT JOIN (SELECT * FROM [dm].[Dim_Product_Pricelist] WHERE Price_List_No='XSJMB0010') P
  ON O.[SKU_ID]=P.SKU_ID
  LEFT JOIN [dm].[Dim_Store] S
  ON O.[Store_ID]=S.Store_ID  
  LEFT JOIN DM.Dim_Product PP
  ON O.[SKU_ID]=PP.SKU_ID
  GROUP BY  LEFT([Purchase_Datekey],6)       
	       ,S.Sales_Area_CN
 UNION
 SELECT MONTHKEY,AREA,0,CAST(TargetAmt AS FLOAT) AS TARGET 
 FROM [Foodunion].[dm].[Fct_Sales_SellInTarget_ByKAarea] WHERE [KA]='YH' AND TargetAmt IS NOT NULL) T
 GROUP BY Datekey,Sales_Area_CN
...
SELECT YearMonth,
	   ...
	   CASE WHEN ISNULL(SI.Target,0)=0 THEN 0 ELSE SI.Amount/SI.Target END Arch_SellIn
FROM #SellOut AS SO
LEFT JOIN #SellIn AS SI
ON SO.YearMonth=SI.Datekey AND SO.Account_Area_CN=SI.Sales_Area_CN

...
CREATE PROC [rpt].[SP_O2O_Employee_Commission_Update]
AS
BEGIN
    ...
    FROM (SELECT 
		...
		CROSS JOIN (SELECT DISTINCT Monthkey FROM dm.Dim_Calendar) b
		where b.Monthkey <= CONVERT(VARCHAR(6),GETDATE(),112)
		) a
	JOIN (
		SELECT 
				tol.Monthkey,
			...
			FROM (SELECT 
					...
					FROM  (SELECT 
								CONVERT(VARCHAR(6),sob.order_close_time,112) AS Monthkey,
							...
							GROUP BY CONVERT(VARCHAR(6),sob.order_close_time,112),sob.Fenxiao_Employee_ID
							...
							WHERE sob.pay_status = '已支付'
								...
								AND sob.order_close_time IS NOT NULL		
								AND sob.Order_Create_Time > ='2019-8-1'			 --从2019-8月开始从新计算佣金						 
							GROUP BY CONVERT(VARCHAR(6),sob.order_close_time,112),sob.Operator_Employee_ID
...
END
	...
	--select top 10000 * from dm.Fct_O2O_Order_Base_info 
	--where Express_Type='同城配送'
	--and order_close_time between '2019-9-1' and '2019-10-31'
	--and Consign_Time is null
	--order by 1 desc 

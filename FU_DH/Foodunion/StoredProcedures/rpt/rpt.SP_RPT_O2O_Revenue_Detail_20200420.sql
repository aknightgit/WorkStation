...
CREATE PROC [rpt].[SP_RPT_O2O_Revenue_Detail_20200420]
AS
BEGIN
	...
	LEFT JOIN [dm].[Fct_O2O_Order_Detail_bySKU] od WITH(NOLOCK) ON ob.Order_ID=od.Order_ID     --将[Fct_O2O_Order_Detail_info] 替换成[Fct_O2O_Order_Detail_bySKU]   Justin 2020-04-20
	LEFT JOIN (
	...
	SELECT od.Revenue_Time AS [Revenue Date]
		  ...
		  ,CASE WHEN ISNULL(od.MAPPING_SKU_ID,'') IN ('','寄生单') OR od.Order_Status = 'TRADE_CLOSED' THEN NULL ELSE (CASE WHEN om.[OutSKUID] IS NOT NULL THEN od.QTY ELSE od.QTY * od.pcs_cnt END) END AS [Qty, sales unit]
		  ,CAST(od.Income_Amount*od.[PriceRatio] AS DECIMAL(18,2)) AS [Actual Order amount]
		  ...
		  ,(CASE WHEN om.[OutSKUID] IS NOT NULL THEN od.QTY ELSE od.QTY * od.pcs_cnt END) AS [商品数量]
		  ,od.QTY AS [件]
	...
	--WHERE Order_No='E20200407113953065700001'

	--SELECT * FROM  #orderdetail WHERE [Order Number]='E20200225161140033300001'
	...
	--WHERE [Order Number]='E20200225161140033300001'

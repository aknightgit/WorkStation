...
CREATE PROC [rpt].[SP_O2O_OrderRecon_Detail_Update]
AS
BEGIN
	...
	LEFT JOIN (
		SELECT DISTINCT Monthkey,Month_Name,Month_Name_Short
		FROM dm.Dim_Calendar
		)c ON CONVERT(VARCHAR(6),CAST(dbo.split(yr.Period,'-',1) AS DATE),112)=C.Monthkey
	...
	WHERE t2.[订单号] IS NULL AND tmp.[交易成功时间] IS NULL;
	...
	FROM #orderdetail tmp
	JOIN (
		SELECT [订单号],MAX([当前累计配送次数]) AS [当前累计配送次数]
		FROM #orderdetail  
		WHERE [订单类型]<>'Normal'
		GROUP BY [订单号]
		)t3 ON tmp.[订单号]=t3.[订单号] AND tmp.[期数]>t3.[当前累计配送次数] AND tmp.[交易成功时间] IS NULL;
	...
	SELECT * 
        ...
        ,'[rpt].[SP_O2O_OrderRecon_Detail_Update]' as [Update_By]
	INTO rpt.O2O_OrderRecon_Detail FROM #orderdetail ORDER BY [入账期间],[订单号],[商品名称],[商品编码]

	--WHERE [订单号]='E20190731123136006900015'
	--CREATE CLUSTERED index inx_O2O_OrderRecon_Detail on rpt.O2O_OrderRecon_Detail([入账期间],[订单号],[商品名称],[商品编码])
	;

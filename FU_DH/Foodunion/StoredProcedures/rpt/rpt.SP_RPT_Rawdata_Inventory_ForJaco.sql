...
CREATE proc [rpt].[SP_RPT_Rawdata_Inventory_ForJaco]
AS
BEGIN
	...
	SELECT 
		...
		,w.Org_Group AS 'Site'
		,w.Tag AS 'Group'
		,i.Stock_Name AS 'WHS Name'
		,w.Warehouse_Name_EN AS 'WHS Name EN'		
		,i.SKU_ID
		,p.Brand_Name AS 'Brand'
		,p.SKU_Name_CN AS 'SKU_Name'
		,p.SKU_Name AS 'SKU_Name_EN'
		...
		,cast(CASE WHEN datediff("d",getdate(),i.Expiry_Date) >0 THEN datediff("d",getdate(),i.Expiry_Date) ELSE 0 END as int) AS 'Days to Expire'
		,cast(CASE WHEN datediff("d",getdate(),i.Expiry_Date) >0 THEN datediff("d",getdate(),i.Expiry_Date)/7 ELSE 0 END as int) AS 'Weeks to Expire'
		,p.Shelf_Life_D AS 'Shelf Life By Days'
		,p.Shelf_Life_D/7 AS'Shelf Life By Weeks'
		,i.Stock_Unit
		,CAST(i.Stock_QTY AS DECIMAL(9,2)) AS Stock_Unit_QTY
		,CAST(Round(i.Sale_QTY * p.Sale_Unit_Weight_KG ,0) AS INT)  AS 'Weight(KG)'
	...
	AND i.Datekey = (SELECT MAX(Datekey) FROM [dm].[Fct_ERP_Stock_Inventory]  with(NOLOCK))
	AND w.Tag='1 - Actual FG'
	...
	--	--,i.* 
	--FROM dm.[Fct_RDC_Inventory] i with(NOLOCK)
	--JOIN dm.Dim_Product p with(NOLOCK) on i.SKU_ID=p.SKU_ID 
	...
	--AND i.Inventory_DT = (SELECT MAX(Inventory_DT) FROM [dm].[Fct_RDC_Inventory]  with(NOLOCK))

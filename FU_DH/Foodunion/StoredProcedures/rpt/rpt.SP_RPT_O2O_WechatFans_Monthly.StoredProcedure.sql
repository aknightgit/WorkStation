USE [Foodunion]
GO
DROP PROCEDURE [rpt].[SP_RPT_O2O_WechatFans_Monthly]
GO
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [rpt].[SP_RPT_O2O_WechatFans_Monthly]
AS
BEGIN 
...
SELECT Year_Month AS YearMonth
	  ,MAX(Date_NM) AS LastDay
	  INTO #LastDay
FROM FU_EDW.Dim_Calendar
WHERE Date_ID BETWEEN 20190301 AND CONVERT(VARCHAR(8),GETDATE(),112)
GROUP BY Year_Month
	...
	--ORDER BY 1 DESC
		...
		SELECT LEFT(a.Date_ID,6) AS YearMonth
			...
			,(SELECT ISNULL(SUM(b.TotalFansCnt_1),MAX(a.TotalFansCnt)) FROM CTE_sum b WHERE DATEADD(DAY,-1,CAST(b.Date_ID AS VARCHAR)) = ld.LastDay) AS TotalFansCnt
			,ld.LastDay
		FROM CTE_sum a
		...
		SELECT c.YearMonth,
			c.LastDay,
			c.NewFansCnt AS NewFansCnt_Month,
			c.UnsubFansCnt AS UnsubFansCnt_Month,
			c.NetIncreFansCnt AS NetIncreFansCnt_Month,
			c.TotalFansCnt AS TotalFansCnt_Month
		FROM  CTE_sum_Month c
/*
	;WITH CTE_C AS(
	SELECT c.Date_ID,
		c.Date_NM,

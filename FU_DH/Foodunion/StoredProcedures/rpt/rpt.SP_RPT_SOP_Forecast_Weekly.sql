...
INTO #YH_SOP
FROM (SELECT DISTINCT CONVERT(VARCHAR(8),Start_of_Week,112) AS Week_ID,Week_of_Year AS Week_Year_NBR,[year] FROM dm.Dim_Calendar) cal
...
SELECT st.Store_City
	  ,st.Store_Province
	  ,CONVERT(VARCHAR(8),Start_of_Week,112) AS Week_ID
	  ,si.SKU_ID
...
LEFT JOIN dm.Dim_Calendar cal ON si.Calendar_DT = cal.Datekey
...
FROM [dm].Fct_YH_Sales_Inventory si
LEFT JOIN dm.Dim_Store st ON si.Store_ID = st.Store_ID
LEFT JOIN dm.Dim_Calendar cal ON si.Calendar_DT<= cal.Datekey AND si.Calendar_DT>CONVERT(VARCHAR(8),DATEADD(DAY,-21,cal.Date_Str),112)
INNER JOIN (SELECT DISTINCT CONVERT(VARCHAR(8),CAST(End_of_Week AS DATE),112) AS WeekEnd FROM dm.Dim_Calendar) calll ON cal.Datekey = calll.WeekEnd
...
LEFT JOIN dm.Dim_Store st ON si.Store_ID = st.Store_ID
LEFT JOIN dm.Dim_Calendar cal ON si.Datekey = cal.Datekey
GROUP BY st.Store_City
		,st.Store_Province
	    ,Start_of_Week
		,cal.End_of_Week
...
FROM [dm].Fct_Kidswant_DailySales si
LEFT JOIN dm.Dim_Store st ON si.Store_ID = st.Store_ID
LEFT JOIN dm.Dim_Calendar cal ON si.Datekey<= cal.Datekey AND si.Datekey>CONVERT(VARCHAR(8),DATEADD(DAY,-21,cal.Date_Str),112)
INNER JOIN (SELECT DISTINCT CONVERT(VARCHAR(8),CAST(End_of_Week AS DATE),112) AS WeekEnd FROM dm.Dim_Calendar) calll ON cal.Datekey = calll.WeekEnd
GROUP BY st.Store_City
		,st.Store_Province
	    ,Start_of_Week

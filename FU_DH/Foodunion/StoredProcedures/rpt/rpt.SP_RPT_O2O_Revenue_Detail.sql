...
CREATE PROC [rpt].[SP_RPT_O2O_Revenue_Detail]
AS
BEGIN
	...
	SELECT 
		 ISNULL(yr.Revenue_Time,OB.Order_Close_Time) AS Revenue_Time
		,ISNULL(yr.Revenue_Type,'订单未入账') AS Revenue_Type
		,ISNULL(yr.Income_Amount,(CASE WHEN ob.Pay_Type IN ('未支付','会员余额','赠品','优惠兑换') THEN 0 ELSE ob.Pay_Amount END)) AS Income_Amount
		,ISNULL(yr.Order_No,ob.Order_No) AS Order_No
		,ISNULL(yr.Revenue_Name,od.Product_Name) AS Revenue_Name
		,c.Month_SNM
	...
	INTO #orderdetail1
	FROM #youzan yr
	FULL JOIN [dm].[Fct_O2O_Order_Base_info] ob WITH(NOLOCK) ON yr.Order_No=ob.Order_No
	LEFT JOIN [dm].[Fct_O2O_Order_Detail_info] od WITH(NOLOCK) ON ob.Order_ID=od.Order_ID
	LEFT JOIN (
		SELECT DISTINCT Monthkey AS Year_Month,Month_Name AS Month_EN_NM,Month_Name_Short AS Month_SNM
		FROM dm.Dim_Calendar
		)c ON CONVERT(VARCHAR(6),CAST(ISNULL(yr.Revenue_Time,OB.Order_Close_Time) AS DATE),112)=C.Year_Month
	...
	LEFT JOIN [dm].[tmp_SKU_Mapping_Youzan] mp
	on od.Product_Name=mp.[sku_name]
	--WHERE yr.Order_No IS NOT NULL OR (Order_Status <>'TRADE_CLOSED' and Order_Close_Time is not null AND payment=0)
	WHERE yr.Order_No IS NOT NULL OR (Order_Status <>'TRADE_CLOSED' and Order_Close_Time is not null AND Pay_Amount<>0)
 ...
 ON P.SKU_ID=PP.SKU_ID;
	...
	SELECT od.Revenue_Time AS [Revenue Date]
		  ...
		  ,od.Order_No AS [唯一值]
		  ,od.Product_ID  --增加ID Mapping 福袋规格 2020-02-18 
		  ...
		  ,CASE WHEN od.pay_type in ('礼品卡支付','组合支付') OR od.Remark IN ('礼品卡支付','组合支付') THEN 'Y' ELSE NULL END AS [礼品卡]
		  ...
		  ,CAST(pl.SKU_Price_withTax AS DECIMAL(9,2)) AS [RSP per sales unit]
		  ,CASE WHEN od.Order_Status <>'TRADE_CLOSED' THEN CAST((CASE WHEN om.[OutSKUID] IS NOT NULL THEN od.QTY ELSE od.QTY * od.pcs_cnt END) * pl.SKU_Price_withTax AS DECIMAL(9,2)) ELSE NULL END AS [RSP*数量]
		  ,CAST((CASE WHEN om.[OutSKUID] IS NOT NULL THEN od.QTY ELSE od.QTY * od.pcs_cnt END) * pl.SKU_Price_withTax AS DECIMAL(9,2))
			- CASE WHEN od.Express_Type='无需发货（虚拟商品订单）' OR od.Order_Status = 'TRADE_CLOSED' THEN NULL
			       WHEN od.is_cycle=1 THEN CAST(CAST(od.payment AS DECIMAL(18,2))/od.delivery_cnt AS DECIMAL(18,2)) 
	...
	LEFT JOIN (SELECT DISTINCT [OutSKUID] FROM [Foodunion].[dm].[Dim_Product_OutSKUMapping]
	           UNION SELECT DISTINCT PRODUCT_ID FROM #SKUM) om
	ON ISNULL(od.Product_ID,'')=om.[OutSKUID]
	...
	SELECT od.[Revenue Date]
		  ,od.[Created date]
		  ,CASE WHEN om.[OutSKUID] IS NOT NULL THEN CAST(od.[Actual Order amount]*ISNULL(om.[PriceRatio],1) AS decimal(18,5)) ELSE CAST(od.[Actual Order amount]*ISNULL(om.[PriceRatio],1) AS decimal(18,2)) END AS [Actual Order amount]    --重新计算福袋   Justin 2020-02-18
		  ...
		  ,CASE WHEN od.REPORT_SKU_ID = '寄生单' THEN '99999999'
				...
				WHEN om.[OutSKUID] IS NOT NULL  THEN om.SKU_ID                     --重新计算福袋   Justin 2020-02-18
		  ...
		  ,CASE WHEN od.[Revenue Type] = '退款' THEN NULL
		        WHEN om.SKU_ID IS NOT NULL THEN om.QTY * od.[件]      --重新计算福袋   Justin 2020-02-18
		  ...
		  ,od.[KOL]
		  ,od.[Order Number]
		  ,CASE WHEN om.[OutSKUID] IS NOT NULL THEN CAST(pl.SKU_Price_withTax AS DECIMAL(9,2)) ELSE od.[RSP per sales unit] END AS [RSP per sales unit] --重新计算福袋金额   Justin 2020-02-18
		  ,od.[Month]
		  ,od.[Operator]
		  ,od.[Order Status]
		  ...
		  ,od.[商品发货状态]
		  ,od.[商家订单备注]
		  ,CASE WHEN om.[OutSKUID] IS NOT NULL THEN CAST(od.[期间内配送金额]*ISNULL(om.[PriceRatio],1) AS decimal(18,5)) ELSE CAST(od.[期间内配送金额] AS decimal(18,2)) END  AS [期间内配送金额]   --重新计算福袋金额   Justin 2020-02-18
		  --,od.[RSP*数量]*ISNULL(om.[PriceRatio],1) AS [RSP*数量]
		  --,od.[折扣]*ISNULL(om.[PriceRatio],1) AS [折扣]
		  ,CASE WHEN od.Order_Status <>'TRADE_CLOSED' AND om.[OutSKUID] IS NOT NULL THEN CAST(om.QTY * od.[件] * pl.SKU_Price_withTax AS DECIMAL(9,2)) ELSE [RSP*数量] END AS [RSP*数量]  --重新计算福袋金额   Justin 2020-02-18
		  ,CASE WHEN om.[OutSKUID] IS NOT NULL THEN CAST(om.QTY * od.[件]* pl.SKU_Price_withTax AS DECIMAL(9,2)) - CAST(od.[期间内配送金额]*ISNULL(om.[PriceRatio],1) AS decimal(18,2))  --重新计算福袋金额   Justin 2020-02-18
			ELSE [折扣] END AS [折扣]  
	FROM #orderdetail od 
	LEFT JOIN (SELECT DISTINCT [OutSKUID],[SKU_ID],[PriceRatio],[QTY] FROM [Foodunion].[dm].[Dim_Product_OutSKUMapping]
	          UNION
	           SELECT DISTINCT PRODUCT_ID,SKU_ID,RATIO,QTY FROM #SKUM) om
	--ON ISNULL(od.Product_ID,'')=om.[OutSKUID]
	ON (CASE WHEN ISNULL(od.Product_ID,'')='' THEN OD.SKU_ID ELSE od.Product_ID END)=om.[OutSKUID]
	LEFT JOIN [dm].[Dim_Product_Pricelist] pl ON pl.Price_List_Name='统一供价' AND ISNULL(om.SKU_ID,od.SKU_ID)=pl.SKU_ID
	--ORDER BY od.[Revenue Date] DESC

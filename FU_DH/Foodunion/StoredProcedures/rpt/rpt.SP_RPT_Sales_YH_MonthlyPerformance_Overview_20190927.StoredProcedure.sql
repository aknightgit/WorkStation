USE [Foodunion]
GO
DROP PROCEDURE [rpt].[SP_RPT_Sales_YH_MonthlyPerformance_Overview_20190927]
GO
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [rpt].[SP_RPT_Sales_YH_MonthlyPerformance_Overview_20190927]
AS BEGIN 
...
SELECT  
	...
	,(SELECT SUM(cds2.Sales_AMT) FROM dm.Fct_KAStore_DailySalesInventory cds2 LEFT JOIN #LastDay ld ON LEFT(cds2.Datekey,6) = ld.YearMonth JOIN dm.Dim_Store ds2 ON ds2.Store_ID=cds2.Store_ID WHERE CASE WHEN ds.Account_Area_CN NOT IN ('重庆大区','四川大区','北京大区','福建大区','华东大区','安徽大区') THEN '其他' ELSE LEFT(ds.Account_Area_CN,2) END
...
SELECT  
	...
	,LEFT(kds.Datekey,6) AS MonthKey
	,COUNT(DISTINCT Datekey) AS Days_Cnt
	,(SELECT SUM(CASE WHEN prod.Product_Sort='Ambient' THEN cds2.Inventory_Qty ELSE 0 END) FROM dm.Fct_KAStore_DailySalesInventory cds2 LEFT JOIN dm.Dim_Product prod ON cds2.SKU_ID = prod.SKU_ID LEFT JOIN #LastDay ld ON LEFT(cds2.Datekey,6) = ld.YearMonth JOIN dm.Dim_Store ds2 ON ds2.Store_ID=cds2.Store_ID WHERE CASE WHEN ds.Account_Area_CN NOT IN ('重庆大区','四川大区','北京大区','福建大区','华东大区','安徽大区') THEN '其他' ELSE LEFT(ds.Account_Area_CN,2) END
	 =CASE WHEN ds2.Account_Area_CN NOT IN ('重庆大区','四川大区','北京大区','福建大区','华东大区','安徽大区') THEN '其他' ELSE LEFT(ds2.Account_Area_CN,2) END

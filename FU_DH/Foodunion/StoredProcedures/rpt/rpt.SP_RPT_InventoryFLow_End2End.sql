...
BEGIN
	...
	LEFT JOIN ( ----IN 进货
		...
		FROM(
			...
			FROM [dm].[Fct_ERP_Stock_InStock] os
			JOIN [dm].[Fct_ERP_Stock_InStockEntry] ose ON os.InStock_ID=ose.InStock_ID
			WHERE   os.Bill_Type='标准采购入库' AND  ose.Stock_Name ='JV2工厂成品在途总仓' --增加 Bill_Type='标准采购入库' --JV2工厂成品在途总仓   ----Justin 2019-12-11
				GROUP BY ose.Stock_Name
				,ose.SKU_ID
				,ose.LOT
			UNION     	
		    SELECT RSE.[Stock_Name]
				  ,RSE.[SKU_ID]
				  ,RSE.[LOT]
				  ,MAX(RSE.[Sale_Unit]) AS Sale_Unit
				  ,SUM(RSE.[Sale_Unit_QTY])  AS Sale_QTY     
			  FROM [dm].[Fct_ERP_Stock_ReturnStock] RS
			  JOIN [dm].[Fct_ERP_Stock_ReturnStockEntry] RSE
			  ON RS.ReturnStock_ID=RSE.ReturnStock_ID
			  WHERE RS.Bill_Type='标准销售退货单' AND RSE.[Stock_Name] like '%退仓%' --增加 Bill_Type='标准销售退货单' --Stock_Name like '%退仓%' ----Justin 2019-12-11
			  GROUP BY RSE.[Stock_Name]
				  ,RSE.[SKU_ID]
				  ,RSE.[LOT]		
			UNION

...
GO
  ...
  CREATE PROCEDURE [rpt].[SP_RPT_Sales_KA_Monthly]
  

  AS

  BEGIN
  DECLARE @Monthperiod VARCHAR(6);


  SET @Monthperiod=CASE WHEN DAY(GETDATE())=1 THEN CONVERT(VARCHAR(6),DATEADD(MONTH,-1,GETDATE()),112) ELSE CONVERT(VARCHAR(6),GETDATE(),112) END
  --SET @Monthperiod=202010

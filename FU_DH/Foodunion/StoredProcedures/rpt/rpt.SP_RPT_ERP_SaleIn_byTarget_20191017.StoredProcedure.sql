USE [Foodunion]
GO
DROP PROCEDURE [rpt].[SP_RPT_ERP_SaleIn_byTarget_20191017]
GO
SET ANSI_NULLS ON
GO
...
GO
  ...
  CREATE PROC  [rpt].[SP_RPT_ERP_SaleIn_byTarget_20191017]
  as
  BEGIN
	...
	SELECT CASE WHEN ch.Team IN ('OFFLINE','Dragon Team','YH') THEN 1 ELSE 99 END  AS CID
		  ...
		  ,ch.Channel_Category  AS [Channel]
		  ,CASE WHEN ch.Channel_Name_Display='Vanguard' THEN '华润万家Vanguard' ELSE ch.[ERP_Customer_Name] END AS [ERP_Account]
		  ,ISNULL(ch.Channel_Name_Display,ch.[ERP_Customer_Name]) AS [Account]
		  ,ISNULL(ch.[Channel_Handler],'') AS [Handler]
		  ,ch.[Team_Handler] AS [Channel_Handler]
	...
	SELECT 
		...
		,CASE WHEN sit.Customer_Name LIKE '%华润%' THEN '华润万家Vanguard'  
	...
	select top 100 * from [dm].[Fct_Sales_SellInTarget] sit WITH(NOLOCK)
	*/

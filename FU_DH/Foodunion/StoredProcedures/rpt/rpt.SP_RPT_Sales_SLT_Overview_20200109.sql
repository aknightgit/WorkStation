...
CREATE PROCEDURE [rpt].[SP_RPT_Sales_SLT_Overview_20200109]
AS BEGIN 
 ...
 ;WITH dailysales AS(
	...
	SELECT Datekey,
		...
		WHEN Channel_Name_Display IN('CenturyMart','SJLH') THEN 'CM'
		WHEN Channel_Name='KidsWant' THEN 'KW'
		WHEN Channel_Type = 'CP' THEN ch.Channel_Category END,
		--ELSE 'Distributors' END AS Channel,
		'Sell In' AS Item,
		CASE WHEN ch.Channel_Name='YH' OR Channel_Name_Display='Vanguard' OR Channel_Name='KidsWant' OR Channel_Name_Display IN ('CenturyMart','SJLH') THEN 'KA'
...
,monthlytarget AS (
	...
	SELECT  
		Datekey/100
		,CASE WHEN ch.Channel_Category in ('Vanguard','YH','Kidswant','CenturyMart','SJLH') THEN 'KA'	ELSE 'CP' END AS Channel_Group
		,CASE WHEN Customer_Name='富平云商供应链管理有限公司' THEN 'YH'
			...
			WHEN ch.Channel_Category IN('CenturyMart','SJLH') THEN 'CM'
			WHEN Channel_Type = 'CP' THEN dc.Channel_Category
			ELSE 'Distributors' END AS Channel
		,'Sell In' AS Item
		,SUM(Target_AMT) AS MonthlyTarget
		,CASE WHEN Customer_Name='富平云商供应链管理有限公司'OR ch.Channel_Category='Kidswant' OR ch.Channel_Category='Vanguard' OR ch.Channel_Category IN('CenturyMart','SJLH') THEN 'CM'
			WHEN Channel_Type = 'CP' THEN 'CP' END
	...
	GROUP BY Datekey/100
		,CASE WHEN ch.Channel_Category in ('Vanguard','YH','Kidswant','CenturyMart','SJLH') THEN 'KA'	ELSE 'CP' END 
		,CASE WHEN Customer_Name='富平云商供应链管理有限公司' THEN 'YH'
		    WHEN ch.Channel_Category='Kidswant' THEN 'KW'
			WHEN ch.Channel_Category='Vanguard' THEN 'VG'
			WHEN ch.Channel_Category IN ('CenturyMart','SJLH') THEN 'CM'
	...
	SELECT  
		Monthkey
		,CASE WHEN Channel_Category_Name in ('Vanguard','YH','Kidswant','CenturyMart','SJLH') THEN 'KA'	ELSE 'CP' END AS Channel_Group
		,CASE WHEN ERP_Customer_Name='富平云商供应链管理有限公司' THEN 'YH'
			...
			WHEN Channel_Category_Name IN ('CenturyMart','SJLH') THEN 'CM'
		...
		,CASE WHEN ERP_Customer_Name='富平云商供应链管理有限公司'OR Channel_Category_Name='Vanguard'OR Channel_Category_Name='Kidswant' OR Channel_Category_Name IN ('CenturyMart','SJLH') THEN 'KA' WHEN Channel_Type = 'CP' THEN 'CP' END AS Channel_Group
	FROM [dm].[Fct_Sales_SellInTarget_ByChannel] ch
	WHERE MonthKey>=201910 AND Team in ('Dragon Team','Offline','YH','MKT')
	GROUP BY Monthkey
		,CASE WHEN Channel_Category_Name in ('Vanguard','YH','Kidswant','CenturyMart','SJLH') THEN 'KA'	ELSE 'CP' END
		,CASE WHEN ERP_Customer_Name='富平云商供应链管理有限公司' THEN 'YH'
			WHEN Channel_Category_Name='Vanguard' THEN 'VG'
			WHEN Channel_Category_Name='Kidswant' THEN 'KW'
			WHEN Channel_Category_Name IN ('CenturyMart','SJLH') THEN 'CM'
			WHEN Channel_Type = 'CP' THEN Channel_Category_Name
			ELSE 'Distributors' END
		,CASE WHEN ERP_Customer_Name='富平云商供应链管理有限公司'OR Channel_Category_Name='Vanguard'OR Channel_Category_Name='Kidswant' OR Channel_Category_Name IN ('CenturyMart','SJLH') THEN 'KA' WHEN Channel_Type = 'CP' THEN 'CP' END

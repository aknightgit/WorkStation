...
GO
CREATE PROCEDURE [rpt].[SP_RPT_Sales_SLT_DPandSell_20191106]
AS BEGIN 
...
,monthlytargetsellin AS (	
	...
	SELECT  
		...
		,CASE WHEN Channel_Category in ('Vanguard','YH') THEN 'KA'	ELSE 'CP' END AS Channel_Group
		,CASE WHEN Customer_Name='富平云商供应链管理有限公司' THEN 'YH'
	...
	GROUP BY Datekey/100
		,CASE WHEN Channel_Category in ('Vanguard','YH') THEN 'KA'	ELSE 'CP' END 
		,CASE WHEN Customer_Name='富平云商供应链管理有限公司' THEN 'YH'
			WHEN Channel_Category='Vanguard' THEN 'VG'
			ELSE 'CP' END
	...
	SELECT  
		Monthkey
		,CASE WHEN Channel_Category_Name in ('Vanguard','YH') THEN 'KA'	ELSE 'CP' END AS Channel_Group
	...
	WHERE MonthKey>=201910
	GROUP BY Monthkey
		,CASE WHEN Channel_Category_Name in ('Vanguard','YH') THEN 'KA'	ELSE 'CP' END 
		,CASE WHEN ERP_Customer_Name='富平云商供应链管理有限公司' THEN 'YH'
			WHEN Channel_Category_Name='Vanguard' THEN 'VG'
			ELSE 'CP' END
	)

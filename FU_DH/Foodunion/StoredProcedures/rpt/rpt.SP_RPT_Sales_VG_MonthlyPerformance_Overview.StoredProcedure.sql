USE [Foodunion]
GO
DROP PROCEDURE [rpt].[SP_RPT_Sales_VG_MonthlyPerformance_Overview]
GO
SET ANSI_NULLS ON
GO
...
SELECT   CASE WHEN ds.Account_Area_CN = '07苏果' THEN '苏果' WHEN ISNULL(ds.Store_Province,'') NOT IN ('天津市','陕西省','江西省','浙江省') THEN '其他' ELSE LEFT(ds.Store_Province,2) END AS Store_Province
		,LEFT(Datekey,6) AS [MonthKey]
		,COUNT(DISTINCT Datekey) AS Days_Cnt
		,SUM( Gross_Sale_Value) AS Sales_AMT
		,(SELECT SUM(Gross_Sale_Value) FROM dm.Fct_CRV_DailySales cds2 LEFT JOIN #LastDay ld ON LEFT(cds2.Datekey,6) = ld.YearMonth JOIN dm.Dim_Store ds2 ON ds2.Store_ID=cds2.Store_ID WHERE ds2.Channel_Account='Vanguard' AND CASE WHEN ds.Account_Area_CN = '07苏果' THEN '苏果' WHEN ISNULL(ds.Store_Province,'') NOT IN ('天津市','陕西省','江西省','浙江省') THEN '其他' 
		 ELSE LEFT(ds.Store_Province,2) END =CASE WHEN ds2.Account_Area_CN = '07苏果' THEN '苏果' WHEN ISNULL(ds2.Store_Province,'') NOT IN ('天津市','陕西省','江西省','浙江省') THEN '其他' 
...
GROUP BY  CASE WHEN ds.Account_Area_CN = '07苏果' THEN '苏果' WHEN ISNULL(ds.Store_Province,'') NOT IN ('天津市','陕西省','江西省','浙江省') THEN '其他' 
...
SELECT   LEFT(cdy.Datekey,6) YearMonth
		,CASE WHEN ds.Account_Area_CN = '07苏果' THEN '苏果' WHEN ISNULL(ds.Store_Province,'') NOT IN ('天津市','陕西省','江西省','浙江省') THEN '其他' ELSE LEFT(ds.Store_Province,2) END AS Store_Province
		,(SELECT SUM(CASE WHEN prod2.Product_Sort='Fresh' THEN cds2.Qty ELSE 0 END) FROM dm.Fct_CRV_DailyInventory cds2 LEFT JOIN dm.Dim_Product prod2 ON cds2.SKU_ID = prod2.SKU_ID LEFT JOIN #LastDay ld ON LEFT(cds2.Datekey,6) = ld.YearMonth JOIN dm.Dim_Store ds2 ON ds2.Store_ID=cds2.Store_ID WHERE ds2.Channel_Account='Vanguard' AND  CASE WHEN ds.Account_Area_CN = '07苏果' THEN '苏果' WHEN ISNULL(ds.Store_Province,'') NOT IN ('天津市','陕西省','江西省','浙江省') THEN '其他' 
			ELSE LEFT(ds.Store_Province,2) END =CASE WHEN ds2.Account_Area_CN = '07苏果' THEN '苏果' WHEN ISNULL(ds2.Store_Province,'') NOT IN ('天津市','陕西省','江西省','浙江省') THEN '其他' 

USE [Foodunion]
GO
DROP PROCEDURE [rpt].[SP_RPT_Sales_YH_MonthlyPerformance_Overview_20190926]
GO
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [rpt].[SP_RPT_Sales_YH_MonthlyPerformance_Overview_20190926]
AS BEGIN 
	...
	SELECT 
		...
		,CASE WHEN tar.Sales_Target>0 THEN MAX(tm.TM_Sales)/tar.Sales_Target ELSE NULL END AS Arch  --进度
	...
	LEFT JOIN #dailysales yt ON yt.Account_Area_CN=tar.Account_Area_CN AND yt.Datekey = (SELECT MAX(Datekey) FROM dm.Fct_KAStore_DailySalesInventory WITH(NOLOCK) WHERE Store_ID LIKE 'YH%')
	...
	LEFT JOIN (
		...
		WHERE ds.Channel_Account='YH'
		AND Datekey=(SELECT MAX(Datekey) FROM dm.Fct_KAStore_DailySalesInventory WITH(NOLOCK) WHERE Store_ID LIKE 'YH%')
		GROUP BY  CASE WHEN ds.Account_Area_CN NOT IN ('重庆大区','四川大区','北京大区','福建大区','华东大区','安徽大区') THEN '其他' 
		ELSE LEFT(ds.Account_Area_CN,2) END 

...
CREATE PROCEDURE [rpt].[SP_RPT_Sales_VG_MonthlyPerformance_Overview]
AS BEGIN 
-------------------------------获取事实表中每个月最后一天
...
SELECT   CASE WHEN ds.Account_Area_CN = '07苏果' THEN '苏果' WHEN ISNULL(ds.Sales_Area_CN,'') NOT IN ('天津','陕西','江西','浙江','乐购','苏果') THEN '其他' ELSE LEFT(ds.Sales_Area_CN,2) END AS Store_Province
		...
		,(SELECT SUM(Gross_Sale_Value) FROM dm.Fct_CRV_DailySales cds2 LEFT JOIN #LastDay ld ON LEFT(cds2.Datekey,6) = ld.YearMonth 
			JOIN dm.Dim_Store ds2 ON ds2.Store_ID=cds2.Store_ID 
			WHERE ds2.Channel_Account='Vanguard' 
			AND CASE WHEN ds.Account_Area_CN = '07苏果' THEN '苏果' WHEN ISNULL(ds.Sales_Area_CN,'') NOT IN ('天津','陕西','江西','浙江','乐购','苏果') THEN '其他' 
		 ...
		 ELSE LEFT(ds2.Sales_Area_CN,2) END AND cds2.Datekey = ld.LastDay AND LEFT(cds2.Datekey,6) = LEFT(cds.Datekey,6)
		 ) AS Sales_Amt_LD
...
SELECT'其他','0','201911' UNION
SELECT'江西','130000','201912'UNION
SELECT'天津','130000','201912'UNION
SELECT'陕西','120000','201912'UNION
SELECT'浙江','170000','201912'UNION
SELECT'苏果','140000','201912'UNION
SELECT'乐购','60000','201912'UNION
SELECT'其他','0','201912' UNION
--SELECT'江西','170000','202001'UNION
--SELECT'天津','150000','202001'UNION
--SELECT'陕西','155000','202001'UNION
--SELECT'浙江','160000','202001'UNION
--SELECT'苏果','170000','202001'UNION
--SELECT'乐购','95000','202001'UNION
--SELECT'其他','0','202001' 
select Area,cast(TargetAmt as decimal(9,2)),Monthkey from dm.Fct_Sales_SellOutTarget_ByKAarea  --从202001开始 指标由hardcode变成指标数据从dm.Fct_Sales_SellOutTarget_ByKAarea 读取。 Justin-2019-01-06
where Monthkey>=202001 and ka='VG'
UNION
select DISTINCT '其他',0,Monthkey from dm.Fct_Sales_SellOutTarget_ByKAarea 
where Monthkey>=202001 and ka='VG'
...
SELECT   LEFT(cdy.Datekey,6) YearMonth
		...
		,(SELECT SUM(CASE WHEN prod2.Product_Sort='Fresh' THEN cds2.Qty ELSE 0 END) FROM dm.Fct_CRV_DailyInventory cds2 
		LEFT JOIN dm.Dim_Product prod2 ON cds2.SKU_ID = prod2.SKU_ID 
		LEFT JOIN #LastDay ld ON LEFT(cds2.Datekey,6) = ld.YearMonth 
		JOIN dm.Dim_Store ds2 ON ds2.Store_ID=cds2.Store_ID WHERE ds2.Channel_Account='Vanguard' 
		AND  CASE WHEN ds.Account_Area_CN = '07苏果' THEN '苏果' WHEN ISNULL(ds.Sales_Area_CN,'') NOT IN ('天津','陕西','江西','浙江','乐购','苏果') THEN '其他' 
			ELSE LEFT(ds.Sales_Area_CN,2) END =CASE WHEN ds2.Account_Area_CN = '07苏果' THEN '苏果' WHEN ISNULL(ds2.Sales_Area_CN,'') NOT IN ('天津','陕西','江西','浙江','乐购','苏果') THEN '其他' 
			ELSE LEFT(ds2.Sales_Area_CN,2) END AND cds2.Datekey = ld.LastDay AND LEFT(cds2.Datekey,6) = LEFT(cdy.Datekey,6) ) AS Fresh_INV
		,(SELECT SUM(CASE WHEN prod2.Product_Sort='Ambient' THEN cds2.Qty ELSE 0 END) FROM dm.Fct_CRV_DailyInventory cds2 
		LEFT JOIN dm.Dim_Product prod2 ON cds2.SKU_ID = prod2.SKU_ID 
		LEFT JOIN #LastDay ld ON LEFT(cds2.Datekey,6) = ld.YearMonth 
		JOIN dm.Dim_Store ds2 ON ds2.Store_ID=cds2.Store_ID WHERE ds2.Channel_Account='Vanguard' 
		AND CASE WHEN ds.Account_Area_CN = '07苏果' THEN '苏果' WHEN ISNULL(ds.Sales_Area_CN,'') NOT IN ('天津','陕西','江西','浙江','乐购','苏果') THEN '其他' 
...
SELECT 
	...
	,MAX(dss.Days_Cnt) AS Days_Cnt
	,MAX(dsslm.Days_Cnt) AS Days_Cnt_LM
...
LEFT JOIN (SELECT Monthkey AS Year_Month,COUNT(1) TMD FROM dm.Dim_Calendar WITH(NOLOCK) GROUP BY Monthkey)dct ON inv.YearMonth = dct.Year_Month

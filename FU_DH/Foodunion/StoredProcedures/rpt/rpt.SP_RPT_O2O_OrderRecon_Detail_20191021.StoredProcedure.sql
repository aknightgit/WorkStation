USE [Foodunion]
GO
DROP PROCEDURE [rpt].[SP_RPT_O2O_OrderRecon_Detail_20191021]
GO
SET ANSI_NULLS ON
GO
...
CREATE PROC [rpt].[SP_RPT_O2O_OrderRecon_Detail_20191021]
AS
BEGIN
	...
	SELECT 
		...
		,CASE WHEN ob.is_cycle=1 AND ob.Order_Status <>'TRADE_CLOSED' THEN od.delivery_cnt ELSE NULL END AS [期数]
		,CASE WHEN ob.is_cycle=1 AND ob.Order_Status <>'TRADE_CLOSED' THEN od.pcs_cnt * od.QTY ELSE NULL END  AS [每次数量]
		,CASE WHEN ob.is_cycle=1 AND ob.Order_Status <>'TRADE_CLOSED' THEN CAST(CAST(od.payment AS DECIMAL(18,2))/od.delivery_cnt AS DECIMAL(18,2)) ELSE NULL END AS [每次金额]
		,CASE WHEN ob.is_cycle=1 AND ob.Order_Status <>'TRADE_CLOSED' THEN yr.Delivery_Cnt ELSE NULL END AS [期间内配送次数]

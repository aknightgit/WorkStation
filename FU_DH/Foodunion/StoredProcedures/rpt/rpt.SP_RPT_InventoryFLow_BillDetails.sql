...
FROM (
	...
	--top 100 
		...
		,P.Sale_Unit AS Sale_Unit                                                                                                 --统一Sale Unit    ----Justin 20191212
		--,stie.QTY*ISNULL(uc.Convert_Rate,1) AS Sale_QTY     
		,(CASE WHEN pu.To_Unit IS NULL THEN stie.QTY ELSE stie.QTY*ISNULL(pu.Convert_Rate,1) END)*ISNULL(uc.Convert_Rate,1) AS Sale_QTY    ----根据 计算出的QTY,计算Sale QTY Justin 20191213                                                                      --统一Sale Unit，计算数量    ----Justin 20191212
	...
	LEFT JOIN dm.Dim_ERP_Unit_ConvertRate uc ON (CASE WHEN pu.To_Unit IS NULL THEN stie.Unit ELSE p.Produce_Unit END)=uc.From_Unit and p.Sale_Unit=uc.To_Unit  --根据 计算出的Unit Mapping--Justin 20191213
	...
	--TOP 100 
		...
		,P.Sale_Unit AS Sale_Unit                                                                                                 --统一Sale Unit    ----Justin 20191212
		--,cast(ose.Sale_Unit_QTY as int)*ISNULL(uc.Convert_Rate,1) AS Sale_QTY   		                                                  --统一Sale Unit，计算数量    ----Justin 20191212
		,(CASE WHEN pu.To_Unit IS NULL THEN ose.Real_QTY ELSE ose.Real_QTY*ISNULL(pu.Convert_Rate,1) END)*ISNULL(uc.Convert_Rate,1) AS Sale_QTY  --Justin 20191213
	...
	LEFT JOIN dm.Dim_ERP_Unit_ConvertRate uc ON (CASE WHEN pu.To_Unit IS NULL THEN ose.Unit ELSE p.Produce_Unit END)=uc.From_Unit and p.Sale_Unit=uc.To_Unit    --Justin 20191213
	...
	--TOP 100 
		...
		,P.Sale_Unit AS Sale_Unit                                                                                                   --统一Sale Unit    ----Justin 20191212
		--,cast(ose.Sale_Unit_QTY as int)*ISNULL(uc.Convert_Rate,1) AS Sale_QTY                                                     --统一Sale Unit，计算数量    ----Justin 20191212
		,cast(ose.QTY as int)*ISNULL(uc.Convert_Rate,1) AS Sale_QTY                                                                 --统一Sale Unit,用Unit QTY计算数量    ----Justin 20191223

USE [Foodunion]
GO
DROP PROCEDURE [rpt].[SP_RPT_ERP_SaleIn_byTarget]
GO
SET ANSI_NULLS ON
GO
...
GO
  ...
  CREATE PROC  [rpt].[SP_RPT_ERP_SaleIn_byTarget]
  as
  BEGIN
	...
	SELECT CASE WHEN ch.Team IN ('OFFLINE','Dragon Team','YH') THEN 1 ELSE 99 END  AS CID
		  ...
		  ,UPPER(ch.[Team]) AS [On_Off_Line]
		  ,ISNULL(ch.Channel_Category,sit.Region) AS [Channel]
		  ,CASE WHEN ch.Channel_Name_Display='Vanguard' THEN '华润万家Vanguard' ELSE ISNULL(ch.[ERP_Customer_Name],sit.Customer_Name) END AS [ERP_Account]
		  ,ISNULL(ch.Channel_Name_Display,ch.[ERP_Customer_Name]) AS [Account]
		  ,ISNULL(ch.[Channel_Handler],'') AS [Handler]
		  ,ISNULL(ch.[Team_Handler],'') AS [Channel_Handler]
	...
	SELECT DISTINCT CASE WHEN coalesce(ch.[Team],sit.Channel,ch2.Team) IN ('OFFLINE','Dragon Team','YH') THEN 1 ELSE 99 END  AS CID
		...
		,sit.Monthkey*100+1 AS [Period]
		,UPPER(coalesce(ch.[Team],sit.Channel,ch2.Team)) AS [On_Off_Line]
		,ISNULL(ch.Channel_Category,sit.Region) AS [Channel]
		,CASE WHEN ch.Channel_Name_Display='Vanguard' THEN '华润万家Vanguard' ELSE ISNULL(ch.[ERP_Customer_Name],sit.Customer_Name) END AS [ERP_Account]
		,ISNULL(ch.Channel_Name_Display,sit.Account_Display_Name) AS [Account]
		,coalesce(ch.[Channel_Handler],ch2.[Channel_Handler],'') AS [Handler]
		,ISNULL(ch.[Team_Handler],ch2.[Team_Handler]) AS [Channel_Handler]
		,sit.Target_Amount AS [Target_AMT]
		,sit.Target_Volumn_MT AS [MT_Target_VOL]
		,0 AS [Actual_AMT]
		,0 AS [Actual_VOL]

...
CREATE PROCEDURE [rpt].[SP_RPT_Sales_YH_MonthlyPerformance_Overview_20200408]
AS BEGIN 
...
--DROP TABLE IF EXISTS #StoreCount;
--SELECT DATEKEY,Store_Province,SUM(CASE WHEN CONT=0 THEN 0 ELSE 1 END ) SC into #StoreCount
--FROM (
--SELECT DATEKEY,KDS.Store_ID,DS.Store_Province,SUM(CASE WHEN INVENTORY_QTY=0 THEN 0 ELSE 1 END) CONT FROM   [dm].[Fct_KAStore_DailySalesInventory] KDS WITH(NOLOCK)          
--			JOIN [dm].[Dim_Store] ds ON kds.Store_ID=ds.Store_ID		
-- 			WHERE ds.Channel_Account='YH'
--			AND DATEKEY= CONVERT(VARCHAR(10),DATEADD(DAY,-1,GETDATE()),112)
--			GROUP BY DATEKEY,KDS.Store_ID,Store_Province) S

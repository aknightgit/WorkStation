...
CREATE PROC [rpt].[SP_RPT_O2O_OrderIncomeRecon]
AS
BEGIN
	...
	--	,yr.Delivery_Cnt AS 'Delivery_Count'
		...
		,CASE WHEN ob.[is_cycle]+od.[delivery_cnt] > 1 THEN yr.Income_Amount ELSE od.[payment]+ob.Shipping_Amount/od.delivery_cnt  END AS Income_Amount
	...
	------------------------粒度改成每个入账
		FROM (SELECT Recon_ID,Order_No,[Period],1 AS Delivery_Cnt ,Income_Amount AS Income_Amount,Recon_Date
			  FROM [dm].[Fct_Youzan_Recon] yr
			  WHERE Recon_Date>='2019-06-01'
			  AND CONVERT(VARCHAR(8),CAST(dbo.split(Period,'-',2) AS DATE),112)<=CONVERT(VARCHAR(8),GETDATE(),112)
		) yr
	...
	LEFT JOIN (
		select distinct Monthkey AS Year_Month,Month_Name AS Month_EN_NM
		from dm.Dim_Calendar)c ON CONVERT(VARCHAR(6),CAST(dbo.split(yr.Period,'-',1) AS DATE),112)=C.Year_Month

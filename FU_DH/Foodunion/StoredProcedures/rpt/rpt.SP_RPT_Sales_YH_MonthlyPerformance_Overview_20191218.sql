...
CREATE PROCEDURE [rpt].[SP_RPT_Sales_YH_MonthlyPerformance_Overview_20191218]
AS BEGIN 
 ...
 --   ,ds.Store_Province 
	...
	,(SELECT SUM(cds2.Sales_AMT) 
		...
		AND CASE WHEN  ds.Account_Area_CN NOT IN ('重庆大区','四川大区','北京大区','福建大区','华东大区','安徽大区','陕西大区','东北大区')
	         AND LEFT(kds.Datekey,6)<'201912'  THEN '其他' WHEN LEFT(kds.Datekey,6)<'201912' THEN LEFT(ds.Account_Area_CN,2) WHEN ds.Account_Area_CN = '东北大区' THEN '东北'  WHEN  LEFT(kds.Datekey,6)>='201912' THEN ISNULL(bk.Area,'其他') ELSE '其他' END 
			=CASE WHEN  ds2.Account_Area_CN NOT IN ('重庆大区','四川大区','北京大区','福建大区','华东大区','安徽大区','陕西大区','东北大区')
	         AND LEFT(cds2.Datekey,6)<'201912'  THEN '其他'  WHEN LEFT(cds2.Datekey,6)<'201912' THEN LEFT(ds2.Account_Area_CN,2) WHEN ds2.Account_Area_CN = '东北大区' THEN '东北' WHEN  LEFT(cds2.Datekey,6)>='201912' THEN ISNULL(bk2.Area,'其他') ELSE '其他' END 
...
SELECT
	 stm.SalesTerritory
	,CASE WHEN  ds.Account_Area_CN NOT IN ('重庆大区','四川大区','北京大区','福建大区','华东大区','安徽大区','陕西大区','东北大区')
	         AND LEFT(kds.Datekey,6)<'201911'  THEN '其他' WHEN LEFT(kds.Datekey,6)<'201911' THEN LEFT(ds.Account_Area_CN,2) WHEN ds.Account_Area_CN = '东北大区' THEN '东北'  WHEN  LEFT(kds.Datekey,6)>='201911' THEN ISNULL(bk.Area,'其他') ELSE '其他' END  AS Account_Area_CN
 --   ,ds.Store_Province 
	,LEFT(kds.Datekey,6) AS MonthKey
	,COUNT(DISTINCT Datekey) AS Days_Cnt
	,(SELECT SUM(cds2.Sales_AMT) 
		...
		WHERE ds2.Channel_Account='YH'  AND stm2.SalesTerritory = stm.SalesTerritory
		AND CASE WHEN  ds.Account_Area_CN NOT IN ('重庆大区','四川大区','北京大区','福建大区','华东大区','安徽大区','陕西大区','东北大区')
	         AND LEFT(kds.Datekey,6)<'201911'  THEN '其他' WHEN LEFT(kds.Datekey,6)<'201911' THEN LEFT(ds.Account_Area_CN,2) WHEN ds.Account_Area_CN = '东北大区' THEN '东北'  WHEN  LEFT(kds.Datekey,6)>='201911' THEN ISNULL(bk.Area,'其他') ELSE '其他' END 
			=CASE WHEN  ds2.Account_Area_CN NOT IN ('重庆大区','四川大区','北京大区','福建大区','华东大区','安徽大区','陕西大区','东北大区')
	         AND LEFT(cds2.Datekey,6)<'201911'  THEN '其他'  WHEN LEFT(cds2.Datekey,6)<'201911' THEN LEFT(ds2.Account_Area_CN,2) WHEN ds2.Account_Area_CN = '东北大区' THEN '东北' WHEN  LEFT(cds2.Datekey,6)>='201911' THEN ISNULL(bk2.Area,'其他') ELSE '其他' END 
		AND cds2.Datekey = ld.LastDay AND LEFT(cds2.Datekey,6) = LEFT(kds.Datekey,6)) AS Sales_Amt_LD
	,SUM(Sales_AMT) AS Sales_AMT
INTO #dailysales_LM
FROM dm.Fct_KAStore_DailySalesInventory kds WITH(NOLOCK)
...
SELECT  
	 stm.SalesTerritory
	,CASE WHEN  ds.Account_Area_CN NOT IN ('重庆大区','四川大区','北京大区','福建大区','华东大区','安徽大区','陕西大区','东北大区')
	 AND LEFT(kds.Datekey,6)<'201912'  THEN '其他' WHEN LEFT(kds.Datekey,6)<'201912' THEN LEFT(ds.Account_Area_CN,2) WHEN ds.Account_Area_CN = '东北大区' THEN '东北'  WHEN  LEFT(kds.Datekey,6)>='201912' THEN ISNULL(bk.Area,'其他') ELSE '其他' END  AS Account_Area_CN
	 ,LEFT(kds.Datekey,6) AS MonthKey
	,COUNT(DISTINCT Datekey) AS Days_Cnt

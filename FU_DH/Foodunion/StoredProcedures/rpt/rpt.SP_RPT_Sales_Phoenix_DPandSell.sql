...
CREATE PROCEDURE [rpt].[SP_RPT_Sales_Phoenix_DPandSell]
AS BEGIN 

	
;WITH DemandPlanning AS(
	SELECT year*100+Month AS Monthkey,CASE WHEN dp.Channel='CP' THEN 'Distributor' ELSE dp.Channel END Channel,p.Product_Sort,sum(volume) AS DP_Vol
...
,monthlytargetsellin AS (	
	...
	SELECT  
		Datekey/100 AS Monthkey
		,CASE WHEN Channel_Category in ('Vanguard','YH','Kidswant') THEN 'KA'	ELSE 'Distributor' END AS Channel_Group
		,CASE WHEN Customer_Name='富平云商供应链管理有限公司' THEN 'YH'
	...
	GROUP BY Datekey/100
		,CASE WHEN Channel_Category in ('Vanguard','YH','Kidswant') THEN 'KA'	ELSE 'Distributor' END 
		,CASE WHEN Customer_Name='富平云商供应链管理有限公司' THEN 'YH'
			WHEN Channel_Category='Vanguard' THEN 'VG'
			WHEN Channel_Category='Kidswant' THEN 'KW'

...
CREATE PROC [rpt].[SP_RPT_O2O_Revenue_Detail_20200417]
AS
BEGIN
	...
	SELECT od.[Revenue Date]
		  ...
		  ,od.[KOL]
		  ,od.[Order Number]
		  ,CASE WHEN om.[OutSKUID] IS NOT NULL THEN CAST(pl.SKU_Price AS DECIMAL(9,2)) ELSE od.[RSP per sales unit] END AS [RSP per sales unit] --重新计算福袋金额   Justin 2020-02-18
		  ,od.[Month]
		  ,od.[Operator]
		  ,od.[Order Status]
		  ...
		  ,CASE WHEN od.Order_Status <>'TRADE_CLOSED' AND om.[OutSKUID] IS NOT NULL THEN CAST(om.QTY*od.[商品数量]* od.pcs_cnt * pl.SKU_Price AS DECIMAL(9,2)) ELSE [RSP*数量] END AS [RSP*数量]  --重新计算福袋金额   Justin 2020-02-18
		  ,CASE WHEN om.[OutSKUID] IS NOT NULL THEN CAST(om.QTY*od.[商品数量] * od.pcs_cnt * pl.SKU_Price AS DECIMAL(9,2)) - CAST(od.[期间内配送金额]*ISNULL(om.[PriceRatio],1) AS decimal(18,2))  --重新计算福袋金额   Justin 2020-02-18
			ELSE [折扣] END AS [折扣]	 
	FROM #orderdetail od
	LEFT JOIN (SELECT DISTINCT [OutSKUID],[SKU_ID],[PriceRatio],[QTY] FROM [Foodunion].[dm].[Dim_Product_OutSKUMapping]) om
	ON ISNULL(od.Product_ID,'')=om.[OutSKUID]
	LEFT JOIN [dm].[Dim_Product_Pricelist] pl ON pl.Price_List_Name='统一供价' AND ISNULL(od.SKU_ID,om.SKU_ID)=pl.SKU_ID

...
--select * from dm.Dim_Store a left join 
--dm.Dim_SalesTerritoryMapping b on a.Store_Province LIKE '%'+ b.Province_Short+'%'
--where b.Code is null and a.Store_Province is not null
--select * from dm.Dim_SalesTerritoryMapping
--select tm.SalesTerritory,st.Province,* from [dm].[Fct_Sales_SellOutTarget_ByKAarea] bk
--LEFT JOIN (SELECT  Channel_Account,ISNULL(Sales_Area_CN,Account_Area_CN) AS Area,MAX(Store_Province) AS Province FROM dm.dim_store GROUP BY Channel_Account,ISNULL(Sales_Area_CN,Account_Area_CN)) st ON CASE WHEN bk.KA = 'Kidswant' THEN 'KW' ELSE bk.KA END  = st.Channel_Account AND st.Area LIKE '%'+bk.Area+'%'
--LEFT JOIN dm.Dim_SalesTerritoryMapping tm ON st.Province LIKE '%'+tm.Province_Short+'%'
--select * from dm.dim_store where store_id like '%KW%' where Store_City like '%锡常%'
CREATE PROCEDURE [rpt].[SP_RPT_Sales_SLT_Overview_20191213]
AS BEGIN 
 ...
 ;WITH dailysales AS(
	...
	SELECT Datekey,
		...
		WHEN Channel_Name_Display='SJLH' THEN 'CM'
		WHEN Channel_Name='KidsWant' THEN 'KW'
		WHEN Channel_Type = 'CP' THEN ch.Channel_Category END,
		--ELSE 'Distributors' END AS Channel,
		'Sell In' AS Item,
		CASE WHEN ch.Channel_Name='YH' OR Channel_Name_Display='Vanguard' OR Channel_Name='KidsWant' OR Channel_Name_Display='SJLH' THEN 'KA'
...
,monthlytarget AS (
	...
	--------------------------------201912后用[dm].[Fct_KAStore_SalesTarget_Monthly]表里的target
	SELECT Monthkey,'KA' AS Channel_Group,CASE KA WHEN 'Vanguard' THEN 'VG' WHEN 'Kidswant' THEN 'KW' ELSE KA END AS Channel,'Sell Out' AS Item,SUM(CAST(TargetAmt AS DECIMAL(20,10)))/1000 AS MonthlyTarget,tm.SalesTerritory
	FROM [dm].[Fct_Sales_SellOutTarget_ByKAarea] bk
	LEFT JOIN (SELECT  Channel_Account,ISNULL(Sales_Area_CN,Account_Area_CN) AS Area,MAX(Store_Province) AS Province FROM dm.dim_store GROUP BY Channel_Account,ISNULL(Sales_Area_CN,Account_Area_CN)) st ON CASE WHEN bk.KA = 'Kidswant' THEN 'KW' WHEN bk.KA = 'CM' THEN 'CenturyMart' ELSE bk.KA END  = st.Channel_Account AND st.Area LIKE '%'+bk.Area+'%'
	LEFT JOIN dm.Dim_SalesTerritoryMapping tm ON st.Province LIKE '%'+tm.Province_Short+'%'
	WHERE Monthkey >= '201212'
	GROUP BY Monthkey,CASE KA WHEN 'Vanguard' THEN 'VG' WHEN 'Kidswant' THEN 'KW' ELSE KA END ,tm.SalesTerritory
	UNION


	SELECT  
		Datekey/100
		,CASE WHEN ch.Channel_Category in ('Vanguard','YH','Kidswant','SJLH') THEN 'KA'	ELSE 'CP' END AS Channel_Group
		,CASE WHEN Customer_Name='富平云商供应链管理有限公司' THEN 'YH'
			...
			WHEN ch.Channel_Category='SJLH' THEN 'CM'
			WHEN Channel_Type = 'CP' THEN dc.Channel_Category
			ELSE 'Distributors' END AS Channel
		,'Sell In' AS Item
		,SUM(Target_AMT) AS MonthlyTarget
		,CASE WHEN Customer_Name='富平云商供应链管理有限公司'OR ch.Channel_Category='Kidswant' OR ch.Channel_Category='Vanguard' OR ch.Channel_Category='SJLH' THEN 'CM'
			WHEN Channel_Type = 'CP' THEN 'CP' END
	...
	GROUP BY Datekey/100
		,CASE WHEN ch.Channel_Category in ('Vanguard','YH','Kidswant','SJLH') THEN 'KA'	ELSE 'CP' END 
		,CASE WHEN Customer_Name='富平云商供应链管理有限公司' THEN 'YH'
		    WHEN ch.Channel_Category='Kidswant' THEN 'KW'
			WHEN ch.Channel_Category='Vanguard' THEN 'VG'
			WHEN ch.Channel_Category='SJLH' THEN 'CM'
	...
	SELECT  
		Monthkey
		,CASE WHEN Channel_Category_Name in ('Vanguard','YH','Kidswant','SJLH') THEN 'KA'	ELSE 'CP' END AS Channel_Group
		,CASE WHEN ERP_Customer_Name='富平云商供应链管理有限公司' THEN 'YH'
			...
			WHEN Channel_Category_Name='SJLH' THEN 'CM'
			WHEN Channel_Type = 'CP' THEN Channel_Category_Name
			ELSE 'Distributors' END AS Channel
		--	ELSE 'Distributors' END AS Channel
		,'Sell In' AS Item
		,SUM(ISNULL(Target_Amt_KRMB,Category_Target_Amt_KRMB)) AS MonthlyTarget
		,CASE WHEN ERP_Customer_Name='富平云商供应链管理有限公司'OR Channel_Category_Name='Vanguard'OR Channel_Category_Name='Kidswant' OR Channel_Category_Name='SJLH' THEN 'KA' WHEN Channel_Type = 'CP' THEN 'CP' END AS Channel_Group
	FROM [dm].[Fct_Sales_SellInTarget_ByChannel] ch
	WHERE MonthKey>=201910 AND Team in ('Dragon Team','Offline','YH','MKT')
	GROUP BY Monthkey
		,CASE WHEN Channel_Category_Name in ('Vanguard','YH','Kidswant','SJLH') THEN 'KA'	ELSE 'CP' END
		,CASE WHEN ERP_Customer_Name='富平云商供应链管理有限公司' THEN 'YH'
			WHEN Channel_Category_Name='Vanguard' THEN 'VG'
			WHEN Channel_Category_Name='Kidswant' THEN 'KW'
			WHEN Channel_Category_Name='SJLH' THEN 'CM'
			WHEN Channel_Type = 'CP' THEN Channel_Category_Name
			ELSE 'Distributors' END
		,CASE WHEN ERP_Customer_Name='富平云商供应链管理有限公司'OR Channel_Category_Name='Vanguard'OR Channel_Category_Name='Kidswant' OR Channel_Category_Name='SJLH' THEN 'KA' WHEN Channel_Type = 'CP' THEN 'CP' END
...
LEFT JOIN weeklysales ws ON wt.Yearkey=ws.Year AND wt.Week_Year_NBR=ws.Week_Year_NBR AND wt.Channel=ws.Channel AND wt.Item=ws.Item AND wt.SalesTerritory = ws.SalesTerritory
LEFT JOIN weeklysales lws ON wt.Yearkey=lws.Year AND wt.Week_Year_NBR=lws.Week_Year_NBR+1 AND wt.Channel=lws.Channel  AND wt.Item=lws.Item AND lws.SalesTerritory = wt.SalesTerritory
...
LEFT JOIN monthlytarget mt ON mt.Monthkey=k.Year_Month AND mt.Channel=wt.Channel AND mt.Item=wt.Item AND mt.SalesTerritory = wt.SalesTerritory
LEFT JOIN monthlysales m1 ON m1.Monthkey=k.Year_Month AND m1.Channel=wt.Channel AND m1.Item=wt.Item and m1.SalesTerritory = wt.SalesTerritory

...
CREATE PROCEDURE [rpt].[SP_RPT_Qulouxia_DailySales_20191128]
AS
BEGIN
    ...
    SELECT ISNULL(sales.[DATEKEY],map.Date_ID) AS [DATEKEY]
      ...
      LEFT JOIN dm.Dim_Product prod ON ISNULL(sales.SKU_ID,map.SKU_ID) = prod.SKU_ID
	  LEFT JOIN dm.Dim_Store st ON ISNULL(sales.Store_ID,map.Store_ID) = st.Store_ID

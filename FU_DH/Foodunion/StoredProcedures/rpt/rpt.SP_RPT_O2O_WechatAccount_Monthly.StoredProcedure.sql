USE [Foodunion]
GO
DROP PROCEDURE [rpt].[SP_RPT_O2O_WechatAccount_Monthly]
GO
SET ANSI_NULLS ON
GO
...
WHERE Date_ID BETWEEN 20190226 AND CONVERT(VARCHAR(8),GETDATE(),112)	
GROUP BY Year_Month



	;WITH CTE_C AS(
	SELECT c.Date_ID,ca.Account,
		c.Date_NM,
	...
	--	AND mp_id = '297825819592626176'
		...
		--ORDER by 1 desc;
		)newfan ON c.Date_ID = newfan.Date_ID AND ca.Account=newfan.Account		
	LEFT JOIN (SELECT in_day AS Date_ID,
		case mp_id when '297825819592626176' then 'Foodunion' 
			when '297465504275238912' then 'Lakto'  
			when '325288922244583424' then 'Rasa' 
		...
		GROUP by in_day,mp_id
		)unsub ON c.Date_ID = unsub.Date_ID	AND unsub.Account=ca.Account
	WHERE c.Date_ID BETWEEN 20190226 AND CONVERT(VARCHAR(8),GETDATE(),112)	
	)
--ORDER BY 1 DESC
	,CTE_sum AS(
	...
	SELECT Year_Month
		...
		--  ,NewSub
		  ,0
		  ,NetIncreace
		  ,SUM(NewSub) OVER(ORDER BY Year_Month)
	FROM(
		SELECT 
			c.Year_Month,
			'Member' as Account,
		...
		on c.Date_ID=m.Date_ID
		LEFT JOIN #LastDay ld ON LEFT(c.Date_ID,6) = ld.YearMonth

USE [Foodunion]
GO
DROP PROCEDURE [rpt].[SP_O2O_Order_Base_info_Delivery_Update]
GO
SET ANSI_NULLS ON
GO
...
SELECT [Datekey]
	  ...
	  ,SubscriptionType  
	  ,Delivery_Seq
	  ,CASE WHEN is_cycle = 1 THEN  DATEADD(DAY,Delivery_Seq*7-7,CAST(Date_NM AS DATE)) ELSE Date_NM END AS DeliveryDate
	  ,CASE WHEN is_cycle = 1 THEN CONVERT(VARCHAR(8),DATEADD(DAY,Delivery_Seq*7-7+CASE WHEN Week_Day<=4 THEN 4-Week_Day ELSE 11-Week_Day END,CAST(Date_NM AS DATE)),112) ELSE CONVERT(VARCHAR(8),[Consign_Time],112) END AS DeliveryDateKey
...
FROM(
	...
	LEFT JOIN FU_EDW.Dim_Calendar cal ON bi.Datekey = cal.Date_ID

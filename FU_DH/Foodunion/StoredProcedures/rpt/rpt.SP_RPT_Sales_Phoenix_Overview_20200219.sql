...
CREATE   PROCEDURE [rpt].[SP_RPT_Sales_Phoenix_Overview_20200219]
AS BEGIN 
	...
	SELECT si.DateKey
		  ...
		  ,CASE WHEN Channel_Type='O2O' THEN 'O2O' 
			...
			WHEN dc.Channel_Category='EC-EKA' THEN 'EKA' END AS Channel
		  ,'Sell Out' AS Item
		  ,SUM(Amount) AS Sales
	...
	GROUP BY si.DateKey
			...
			WHEN dc.Channel_Category='EC-PDD' THEN 'PDD' 
			WHEN dc.Channel_Name_Display ='去楼下ZBox' THEN 'ZBox' 
			WHEN dc.Channel_Category='EC-EKA' THEN 'EKA' END
	UNION
	...
	SELECT '202002' AS Monthkey,'O2O' AS Channel,'Sell Out' AS Item,43000  AS MonthlyTarget UNION ALL
	SELECT '202002' AS Monthkey,'Zbox' AS Channel,'Sell Out' AS Item,360000  AS MonthlyTarget

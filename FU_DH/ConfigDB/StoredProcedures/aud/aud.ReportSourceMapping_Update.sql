...
BEGIN	
	...
	join (
		...
		--,*
		from  [Foodunion].dm.[Fct_YH_Sales_Competitive_Product]
		order by Date_ID desc
	)b
	on 1=1
	...
	from  aud.ReportSourceMapping a
	join (
	select top 1 Load_Source,CONVERT(date, Load_DTM, 120 )  as Load_DT from  ods.[ods].[File_Qulouxia_Sales]
	order by Load_DT desc
	)b
	on 1=1
	where Subject_Area='Qulouxia Pos Data';
	--Qulouxia store sku mapping					By:CWC
	update a
	set a.Up_to_Date=CONVERT(varchar(8), Load_DT, 112 ), a.Last_Load_File=b.Load_Source,a.Last_Run_Time=GETDATE(),
		a.Update_by='[aud].[ReportSourceMapping_Update]',
		a.Update_datetime = getdate() --,a.Last_Run_Time=cast(b.Load_DTM as datetime)
	...
	from  aud.ReportSourceMapping a
	join (
	select top 1 Load_Source,CONVERT(date, Load_DTM, 120 )  as Load_DT from  ods.[ods].File_Qulouxia_Store_SKU_Mapping
	order by Load_DT desc
	)b
	on 1=1
	where Subject_Area='Qulouxia Store SKU Mapping';
	--O2O KOL Mapping
	update a
	set a.Up_to_Date=CONVERT(varchar(8), Load_DTM, 112 ), a.Last_Load_File=b.File_NM,a.Last_Run_Time=GETDATE(),
		a.Update_by='[aud].[ReportSourceMapping_Update]',
	...
	from  aud.ReportSourceMapping a
	join (
		select top 1 Load_DTM as Date_ID,Load_Source as File_NM,Load_DTM as Load_DTM FROM ODS.[ods].File_Plant	WHERE Load_Source LIKE '%JV1%'
		order by Load_DTM desc
	)b
	on a.Is_Enabled=1
	where Data_Source_Name='Plant Produce JV1';
	...
	from  aud.ReportSourceMapping a
	join (
		select top 1 Load_DTM as Date_ID,Load_Source as File_NM,Load_DTM as Load_DTM FROM ODS.[ods].File_Plant	WHERE Load_Source LIKE '%JV2%'
		order by Load_DTM desc
	)b
	on a.Is_Enabled=1
	where Data_Source_Name='Plant Produce JV2';
	--KW Pos Data
	update a
	set a.Up_to_Date=CONVERT(varchar(8), Load_DTM, 112 ), a.Last_Load_File=b.File_NM,a.Last_Run_Time=GETDATE(),
		a.Update_by='[aud].[ReportSourceMapping_Update]',
	...
	from  aud.ReportSourceMapping a
	join (
		select top 1 Revenue_Time as Date_ID,Load_Source as File_NM,Load_DTM as Load_DTM FROM ODS.[ods].[File_Youzan_Revenue_Details]
		order by Revenue_Time desc
	)b
	on a.Is_Enabled=1
	where Data_Source_Name='Youzan Revenue Details';

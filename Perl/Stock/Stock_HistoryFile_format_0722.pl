...
my @list = grep {/.*\#\d+.txt$/ && -f "$DATDIR/$_" } readdir(DH) ;
...
#	print @contents;
	...
	my $firstline = shift(@contents);
#	print $firstline;
	my ($stockcode,$stockname, $stocknametail) = ($1,$2,$3) if {$firstline =~ m/(\d+)\|(.*?)\|(.*?)\|/};
#	print $stockcode;
#	print $stockname ;
#	print $stocknametail;
	$stockname = $stockname.$stocknametail unless $stocknametail  eq "日线";
#	print $stockcode;
#	print $stockname;
	for(@contents){		
#		s/^\D+.*//;

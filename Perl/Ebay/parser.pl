...
my ($SEQ_NUM,$SRC_ID,$START_TIME,$END_TIME,$JOB_START,$JOB_FINISH,$COST_TIME,$TOT_TIME,$att_queue);
...
@LOG_FILE=(<$DW_LOG/$JOB_TYPE/$SUBJECT/$TABLE_ID.$JOB_TYPE_ID.$SUBJECT.$HANDLER.$day[0]-*.log>,<$DW_ARC/$JOB_TYPE/$SUBJECT/$TABLE_ID.$JOB_TYPE_ID.$SUBJECT.$HANDLER.$day[0]-*.log>);
$LOG_FILE=$LOG_FILE[-1];
if (@ATT_FILE>1){
...
foreach(@FIELD_ID){
    ...
    $JOB_START=substr($1,-8,8) if($logcontent=~/item_attr_info\.$i\.dat.$SEQ_NUM(.*) GMT/);
...
$TOT_TIME=minus($Now,$START_TIME);
my $TOT_SECOND=(split(/\:/,$TOT_TIME))[0]*3600+(split(/\:/,$TOT_TIME))[1]*60+(split(/\:/,$TOT_TIME))[2];

foreach(@TARGET_TABLE){
    my $seq=$SEQ_NUM-7;
    my ($REC_CT_FILE)=(<$mytmp/$_.record_count.dat.$seq>);
...
printf "%-6s%-12s%-12s%-10s%-10s%-10s%-10s%-8s\n",'Tot',$START_TIME,$Now,$TOT_REC[0],$TOT_REC[1],$TOT_REC[2],$TOT_REC[3],$TOT_TIME;
if($FINISH!=1){
print "$slash1\n";

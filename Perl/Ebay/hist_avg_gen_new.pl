...
#2008-11-22          1.2         Kevin                  add Time.pm
####################################################################################################
...
my (@pars,@TH_FILE,@date,@exclude,@ARC,%IS_FRIDAY);
...
foreach $TH_FILE (@TH_FILE){
    ...
    foreach(@ARC){
		...
		foreach(<ARCLOG>){
		    if($_=~/(..:..)     $TH_FILE/){
			    if($IS_FRIDAY{$date} ne 1){
				    push(@HIST,$1);
				}
				else{
				    my $tmp=sprintf "%02d:%02d",(split(':',$1))[0]-1,(split(':',$1))[1];
					push(@HIST,$tmp);
				}
			}
		}
		...
		my $avg=Time->new(@HIST)->avg;
		my $avg=substr($avg,0,5);
